...
| shared/components | 无业务数据的布局/控件 primitive；业务档位 / locale / 产品壳（store/钱包）不进；跨 tab 产品壳 → `views/dapp/shared/`；窗口级 host（rail/topbar/onboarding/wallet）→ `views/dapp/host/`；入口 → `src/bootstrap/` |
