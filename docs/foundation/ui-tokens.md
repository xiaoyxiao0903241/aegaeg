...
|`tone`|`foreground` · `muted-foreground` · `primary` · `primary-bright` · `success` · `claim` · `inverse` · `inverse-muted`|
|可选|`as`|
...
**Gate**：`text.tsx` variant 键 = **12** · `TextVariant` 联合 = 12 · `tone` = 8
...
`claim` = 领取蓝 `#4a7bec`（Mixed 领取轨、地址链接、共建达成）。**禁止**旧名 `claim-restake`。
...
|`ClaimSplitSlider`|slider `4812:221`|`value` (release%), `onChange`, `aria-label`, `restakeLabel`, `releaseLabel`, `hint`|左复投蓝 / 右领取橙；标签在轨上；thumb 分界上显示两侧数字；CTA 同比例渐变|
...
- `ClaimSplitSlider`：`@radix-ui/react-slider`；左轨 `bg-claim`（复投蓝）· 右轨 `bg-primary`（领取橙）；轨上左 `%`+复投、中提示、右领取+`%`；白底 thumb 内嵌两侧数字（左蓝右橙）；`value`/`onChange` 仍是释放%（`restakePct = 100 − releasePct`）；CTA 背景走 `claimSplitCtaBackgroundImage`（同一套比例，100% 领取纯橙、100% 复投纯蓝）；`aria-label` / 三条文案必填（i18n）。去向卡同色：复投 `claim`、领取 `primary`。
...
- `InlineAlert`（`src/shared/components/inline-alert.tsx`）：`tone` = `destructive`（默认，错误/阻断：浅红 wash）| `notice`（机制说明：贴稿珊瑚软底 `bg-coral-wash`）；**两种均无描边**，正文统一 `muted-foreground`（仅底色区分语义）；`density` = `compact` | `comfortable`；字阶仍走 Text `copy`；**不是** Card surface，**勿**并入 `inverse`。间距（`mt`/`mx`/`mb`）留 call site。共享层禁 `Dapp*` 前缀（产品壳前缀只留 `views/dapp/shared` / `views/dapp/host` / 页袋）。
...
- `ResponsiveDialog`（`dialog.tsx` · `dialogChrome.panel`）：默认浅色卡片壳（`bg-card` · `max-w-md` · H5 顶圆角底抽屉 / PC `rounded-lg` + `shadow-modal-panel` · `px-4 py-5`）；领取 / 赎回 / 滑点等直接用默认；钱包连接·详情仅 `className` 覆盖异形尺寸/圆角/padding。禁业务袋平行 chrome 常量。
- `DialogClose`（`dialog.tsx`）：DApp modal/sheet 关闭钮（浅灰圆底 `bg-muted` + X；无 focus 描边环）；details / slippage / 资产领取·赎回共用；深色壳可 className 覆盖；Connect 仍用 `.aegis-wallet-connect-close`；Home popup 深色圆钮独立；**H5 drawer** 关闭为透明 X（≠ modal close）。
