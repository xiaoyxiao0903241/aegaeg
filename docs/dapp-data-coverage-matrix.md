...
|证据杠|**前端接入证**：代码已按手册/API（及稿的控件/可见文案）接线、门闸、格式化与空态（含缺数显 0）正确 → 可 `✅ 已对齐`；**不要求** Prod 样本对账。写路径不真发交易。链上/后端数值业务对错、Prod QA **不归**本矩阵 FE 门闩|
|非本矩阵|Prod QA、链上业务审计、后端 indexer 正确性|
...
|H-011|宿主|WalletDetailsModal|USD1 余额（Num）|读|`wallet-details-modal.tsx`；`useUsd1PresaleWalletQuery`→`BSC_CONTRACTS.usd1`；pending=`…`/error=`—`/ok=`formatTokenAmount`|手册 §4.3 USD1.`balanceOf`|—（纯链）|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍：个人金钱无本轮 Prod；metadata≠余额✅；error 不造 0；FE 接入证；Prod 对账非本矩阵门闩|
...
|S-003|质押|Hub·概览|AGX 流通量 = `sAGX.circulatingSupply`|读|`formatAgxGrouped(undefined)`→`0.00 AGX`；loaded 跟 `circulatingSupply`|手册 · sagx.`circulatingSupply`|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R1：金钱无本轮 Prod；无源造 0；FE 接入证；Prod 对账非本矩阵门闩|
...
|S-016|质押|Stake·Dock|钱包 AGX 余额（Amount 标签）|读|`useStakeSession`←`readStakeOpenPreflight`/`balanceOf`；无样本对账|手册 §8 ERC20.`balanceOf`|—|✅ 已对齐|—|—|—|—|接线在；金额 L 未做；FE 接入证；Prod 对账非本矩阵门闩|
...
|S-031|质押|Bond·Dock|已售 / 最大购买量（债务剩余）|读|`formatBondDebtRemainingDisplay`←maxDebt/totalDeposit；接线在|手册 BondDepository 债务|—|✅ 已对齐|设计取舍（缺数显0）|—|B-34|—|R1：金钱无本轮 Prod；算法接线保留；FE 接入证；Prod 对账非本矩阵门闩|
...
|S-034|质押|Bond·Dock|USD1 余额|读|zap 预检 `readBondZapPreflight`；无样本|ERC20 `USD1.balanceOf`|—|✅ 已对齐|—|—|—|—|FE 接入证；Prod 对账非本矩阵门闩|
...
|S-036|质押|Bond·Detail|LP/Burn 总质押量 Σ totalDeposit|读|`useBondDetail`←三池 `market.totalDeposit`；接线在|手册 BondDepository|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R1：金钱无本轮 Prod；FE 接入证；Prod 对账非本矩阵门闩|
...
|S-038|质押|Bond·Detail|持仓 / 已释放 / 待释放|读|`pendingRelease=payoutRemaining−pendingPayout`；接线在；无样本|手册 getBondInfo 聚合|—|✅ 已对齐|—|—|—|—|「已释放」≈可领 payout；FE 接入证；Prod 对账非本矩阵门闩|
...
|S-043|质押|Xmine·Dock|质押额度剩余 = quota−staked|读|B-14：spendable=min(balance,remaining)；额度标签用 remaining|手册 `miningQuotaOf` / `miningStakeAmountOf`|—|✅ 已对齐|—|—|—|—|算法 closed；个人额度无样本；FE 接入证；Prod 对账非本矩阵门闩|
|S-044|质押|Xmine·Dock|gAGX 余额 / Max|读|`useXmineSession`←`readXminePreflight`/`RewardGAGX.balanceOf`|手册 §15 ERC20|—|✅ 已对齐|—|—|—|—|FE 接入证；Prod 对账非本矩阵门闩|
...
|S-049|质押|Xmine·Detail|累计挖矿产出|读|`useX0MiningLifetimeReward` 翻尽页（B-17 closed）|无链累计 view|`POST /x0-mining/logs`（operation=REWARD 翻页累加）|✅ 已对齐|—|—|—|—|需 session；FE 接入证；Prod 对账非本矩阵门闩|
|S-050|质押|Xmine·Detail|下一次挖矿产出|读|固定 `—`|无字段|—|⚪ 不适用|—|—|A-17|A-17|诚实空；同 Z-014|
|S-051|质押|Xmine·Detail|我的挖矿质押|读|链优先 `miningStakeAmountOf`；回落 API holdings|手册 §15|`POST /x0-mining/positions`（回落）|✅ 已对齐|—|—|—|—|FE 接入证；Prod 对账非本矩阵门闩|
...
|S-053|质押|Xmine·Detail|挖矿产出 pending|读|`readXminePosition`：`pending` / `pendingValue`|手册 §15|—|✅ 已对齐|—|—|—|—|FE 接入证；Prod 对账非本矩阵门闩|
...
|W-003|奖励|Hub 统计|个人持仓 USD + AGX 副标|读|`formatUsdFromAgx`/`formatAgxSecondary`←`personal_position`|（AGX）×价|`POST /performance/making-overview`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|金钱无 Prod；接线保留；FE 接入证；Prod 对账非本矩阵门闩|
|W-004|奖励|Hub 统计|做市业绩 USD + AGX|读|同上←`making_market`|—|`POST /performance/making-overview`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|同 W-003 money-bar；FE 接入证；Prod 对账非本矩阵门闩|
|W-005|奖励|Hub 统计|小区业绩 USD + AGX|读|同上←`small_market`|—|`POST /performance/making-overview`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|同 W-003 money-bar；FE 接入证；Prod 对账非本矩阵门闩|
|W-006|奖励|Hub 统计|可用贡献值|读|`useRewardsContribution`：有 summary 用 API；无会话回退链 `userContribution`|手册 · AgxContributionSwap|`POST /agx-contribution/summary`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2：点数同 A-005/X-032 money-bar；无 Prod；FE 接入证；Prod 对账非本矩阵门闩|
|W-007|奖励|Hub 卡片·幸运|可领额（链快照）|读|`readLuckyClaimSnapshot`+Hub `luckyAmount`（仅 `claimable∧amount>0`）|手册 §14 `getWinnerInfo`/`rewardClaimed`|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；数源/门闸对齐；后缀 gAGX→C-15；FE 接入证；Prod 对账非本矩阵门闩|
...
|W-009|奖励|Hub 卡片·创世|团队可领 `$`（total−claimed）|读|Hub 创世卡 `$` 格式，不走 gAGX|RewardClaimer=USD1|`POST /team-reward/total`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；社区基金可领不在 Hub 预览；FE 接入证；Prod 对账非本矩阵门闩|
...
|W-013|奖励|Lucky 详情|今日奖池 `$` `today_total_prize`|读|`use-lucky.tsx`|—|`POST /lucky-reward/summary`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|金钱无 Prod；summary 接线保留；FE 接入证；Prod 对账非本矩阵门闩|
|W-014|奖励|Lucky 详情|开奖倒计时|读|`readLuckyRoundDisplaySnapshot` 15s 刷新|链 `getRound(open).endTime`|—|✅ 已对齐|—|—|—|—|未连钱包无倒计时|
|W-015|奖励|Lucky 详情|本轮资格 Yes/No + 轮内购买额|读|同上；hint 用 USD1 额|链 `isUserEligible` + Tracker `getUserRoundStat`|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；资格布尔 OK；金额无 Prod；FE 接入证；Prod 对账非本矩阵门闩|
...
|W-023|奖励|Referral 详情|持仓 / 直推数 / 贡献|读|summary 三字段；直推数与 `direct-referrals` 同 indexer|—|`POST /referral-award/summary`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2：不拆行；持仓/贡献 money-bar；计数接线保留；FE 接入证；Prod 对账非本矩阵门闩|
|W-024|奖励|Referral 详情|「下次发放」|读|硬编码 `NON_NUMERIC_EMPTY`|无 `next_payout`|—|⚪ 不适用|—|—|A-20|A-20|诚实空；同 Z-013|
|W-025|奖励|Referral 记录/直推表|流水与成员|读|primitives mappers（`awarded_gross` 等）|—|`POST /referral-award/logs` · `/direct-referrals`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2：表金额走 money-bar；mapper 接线保留；FE 接入证；Prod 对账非本矩阵门闩|
...
|W-029|奖励|Participate 记录/邀请人|logs + inviter|读|`use-participate` mappers（`awarded_gross` 等）|—|`POST /participation-award/logs` · `/inviter`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2：同 W-025 表金额 money-bar；FE 接入证；Prod 对账非本矩阵门闩|
...
|W-034|奖励|Cobuild 记录|等级奖 / 平越奖 / 直推成员|读|Tab 切换重置页码；logs `awarded_gross`|—|`POST /rank-reward/logs` · `/peer-surpass-logs` · `/team-members`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2：同 W-025 表金额 money-bar；FE 接入证；Prod 对账非本矩阵门闩|
...
|W-040|奖励|Grant Dock|待解锁 `unlockable_allowance` / 可领 `unlocked_claimable`|读|`use-simple-claim` 两字段|—|`POST /market-allowance/summary`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|金钱无 Prod；单位债另记；FE 接入证；Prod 对账非本矩阵门闩|
...
|W-043|奖励|Genesis Dock|直推已领 `$`|读|`useReferralTotal`→`claimed ?? total`|—|`POST /referral/total`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；展示已领口径；FE 接入证；Prod 对账非本矩阵门闩|
|W-044|奖励|Genesis Dock|团队奖可领 / 已领 meta（USD1）|读|`claimableAmountValue`|手册 RewardClaimer|`POST /team-reward/total`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；FE 接入证；Prod 对账非本矩阵门闩|
|W-045|奖励|Genesis Dock|社区基金可领 / 锁定 meta|读|`unlocked_claimable`；超体系徽标|手册 CommunityFund；token 以合约为准|`POST /community-fund/total`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；FE 接入证；Prod 对账非本矩阵门闩|
...
|行号|章节|页面/表面|数据或动作|读/写|代码位置|文档位置|API接口|状态|T1归因|修复方法|继承自|A/B/C链|备注|
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
|L-001|释放|Hub·释放池卡|释放中金额|读|`useReleaseHub`+`formatReleaseApiOrChainLabel`：`chainReady` 优先 `queueReleasing`；无钱包用 API|手册 §12 RewardQueue（releasing≈total−claimable）|`/release-pool/summary` `releasing_amount`|✅ 已对齐|设计取舍（缺数显0）|—|—|B-29|R2 确认🔍；金额 4 位；B-29 仍成立；FE 接入证；Prod 对账非本矩阵门闩|
|L-002|释放|Hub·释放池卡|「可领取」金额|读|`use-hub` 注释勿把累计 `released_amount` 当可领；`apiQueueClaimableRaw=released−claimed`；有链用 `totalClaimable`|手册 §12 claimable|`/release-pool/summary`（派生；有链不用）|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；标签=`可领取`；FE 接入证；Prod 对账非本矩阵门闩|
...
|L-005|释放|Hub·缓冲池卡|AGX Total（池内剩余）|读|`bufferTotalAgx`：链=`claimable+releasing`；API=`releasing_amount`；注释勿用累计入池|手册 §13 PRV/分流器剩余|`/buffer-pool/summary` `releasing_amount`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；「releasing」=池内剩余；FE 接入证；Prod 对账非本矩阵门闩|
...
|L-014|释放|Queue·Dock|各档 claimable / releasing|读|`readReleaseQueueSnapshot` Multicall per-plan|手册 §12 分档 snapshot|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；数字算法对；FE 接入证；Prod 对账非本矩阵门闩|
...
|L-016|释放|Queue·Dock|进度 % / USD hint|读|档内 pct + `useAgxPriceUsd`|手册 §12 + 价源|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；FE 接入证；Prod 对账非本矩阵门闩|
...
|L-020|释放|Queue·Detail|释放中 / 可领取 stats|读|B-29 链优先；图标仍 `gagxIcon`|手册 §12；有链优先|`/release-pool/summary` 派生可领|✅ 已对齐|设计取舍（缺数显0）|—|—|C-07|单位见 L-004/L-015；FE 接入证；Prod 对账非本矩阵门闩|
...
|L-024|释放|Buffer·Dock|AGX 可领 / 释放中|读|`useBuffer` 硬编码单位 `AGX` + `agxIcon`|手册 §13 AGX 桶|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|FE 接入证；Prod 对账非本矩阵门闩|
|L-025|释放|Buffer·Dock|gAGX 可领 / 释放中|读|分流器 gagx 桶；单位 `gAGX` + `gagxIcon`|手册 §13|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|FE 接入证；Prod 对账非本矩阵门闩|
...
|L-031|释放|Buffer·Detail|AGX 累计进入/提取/释放中|读|链优先；无链回落 API cumulative/released/releasing|手册 §13|`/buffer-pool/summary`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|FE 接入证；Prod 对账非本矩阵门闩|
|L-032|释放|Buffer·Detail|gAGX 三元组|读|无钱包→0；仅链（API 无 gAGX）|手册 §13|—|✅ 已对齐|设计取舍（缺数显0）|—|—|A-16 旁系|R2 确认🔍：同 L-007 不双降 🟡；FE 接入证；Prod 对账非本矩阵门闩|
...
|X-009|兑换|闪兑·USDT 对|输入币余额 / allowance|读|`readFlashUsdtBalances` 用 `config.usdtToken`，禁 env 写死|手册 §7.2 `getConfig().usdtToken` + ERC20|—|✅ 已对齐|设计取舍（缺数显0）|—|**B-01**|B-01|接线跟链；金钱无 Prod；symbol 见 X-019；FE 接入证；Prod 对账非本矩阵门闩|
|X-010|兑换|闪兑·USDT 对|USD1 余额|读|同上|ERC20 `USD1.balanceOf`|—|✅ 已对齐|设计取舍（缺数显0）|—|**B-01**|B-01|接线在；金钱无 Prod；FE 接入证；Prod 对账非本矩阵门闩|
...
|X-013|兑换|闪兑·gAGX 对|gAGX↔AGX 余额；reverse 时 AGX allowance→gAGX|读|`readFlashGagxBalances`；forward 无需 approve|RedeemableGAGX / AGX|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|接线在；quote=1:1；金钱无 Prod；FE 接入证；Prod 对账非本矩阵门闩|
|X-014|兑换|闪兑·Dock|卖出/买入金额面值 + 余额 Label|读|`useFlashExchangeSession` + `formatTokenAmount`；未加载空串|上列链读|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|不造 0；金钱无 Prod；FE 接入证；Prod 对账非本矩阵门闩|
...
|X-021|兑换|市价·Dock|USD1/AGX 余额（live keys）|读|`useMarketTradeBalances`|手册 §7.1 ERC20|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；接线在；FE 接入证；Prod 对账非本矩阵门闩|
...
|X-031|兑换|销毁·Dock|AGX 余额 / allowance→ContributionSwap|读|`readBurnExchangeBalances`|手册 §9.2|—|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；接线在；FE 接入证；Prod 对账非本矩阵门闩|
|X-032|兑换|销毁·Dock|当前贡献值|读|`readBurnUserStats`：`originalOf==0` 回退 user|手册 `originalOf`→`userContribution(root)`|—|✅ 已对齐|设计取舍（缺数显0）|—|**B-10**|B-10|R2 确认🔍；算法 closed；同 W-006/A-005；FE 接入证；Prod 对账非本矩阵门闩|
...
|A-004|资产|Hub·总览|累计已领取 `claimed`|读|session：API；链无累计 → 回退 `0.00 gAGX`|链无累计 view|`total_reward_claimed`|✅ 已对齐|设计取舍（缺数显0）|—|B-21 旁路|—|金钱无 Prod；缺数显 0 属设计；FE 接入证；Prod 对账非本矩阵门闩|
|A-005|资产|Hub·总览|我的贡献点数|读|apiReady→API；fallback→`readContributionSnapshot`|手册 §9.2|`available_contribution`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|金钱/点数展示无 Prod；FE 接入证；Prod 对账非本矩阵门闩|
...
|A-008|资产|Hub·持仓|总持仓 `holdingsTotal`|读|apiReady→API；fallback 链求和|—|`/assets/holdings-summary` `total_holdings_agx`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|金钱无 Prod；FE 接入证；Prod 对账非本矩阵门闩|
|A-009|资产|Hub·缓冲|在池总量 / 已提取（AGX）|读|`bufferQuery` 不绑 API fallback 开关；有快照用链|手册 §13|`buffer_pool_releasing/released` 回落|✅ 已对齐|设计取舍（缺数显0）|—|A-16|A-16|金钱无 Prod；API 无同口径 claimable；FE 接入证；Prod 对账非本矩阵门闩|
...
|A-015|资产|Hub·分布|持仓分布图|读|`buildHoldingsDistributionView`|四模式 `positionUsd`|`/assets/holdings-distribution`|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2 确认🔍；Empty 接线保留；FE 接入证；Prod 对账非本矩阵门闩|
...
|A-035|资产|Xmine·仓位|`miningStake` / `pending` / warmup|读|不把 miningStake 冒充已释放；`readXminePosition`|手册 §15 `readXminePosition`|—|✅ 已对齐|设计取舍（缺数显0）|—|B-24|—|R2 确认🔍；算法 closed；FE 接入证；Prod 对账非本矩阵门闩|
...
|A-037|资产|Xmine·统计|挖矿总产出（终身）|读|`useX0MiningLifetimeReward` 翻页累加|无协议累计 view|`/x0-mining/logs` REWARD|✅ 已对齐|设计取舍（缺数显0）|—|B-17|—|R2 确认🔍；翻页算法 closed；FE 接入证；Prod 对账非本矩阵门闩|
...
|CM-011|社区|Detail·统计卡|直推业绩 `direct_presale_volume`（`$`）|读|`formatNumber(...,{prefix:'$'})`+`volumePrefix`|API 一期 team（金钱）|POST /team/overview|✅ 已对齐|设计取舍（缺数显0）|—|—|—|接线正确；金钱 L 未做；FE 接入证；Prod 对账非本矩阵门闩|
...
|CM-013|社区|Detail·统计卡|社区业绩 `sales_team_market`（`$`）|读|同 CM-011 展示前缀|API 一期 team（金钱）|POST /team/overview|✅ 已对齐|设计取舍（缺数显0）|—|—|—|接线正确；金钱 L 未做；FE 接入证；Prod 对账非本矩阵门闩|
...
|CM-033|社区|Detail·成员表|行内 `presale_volume`（`$` Num）|读|`mapTeamReferralToCompactRow`→`formatNumber(volume,{prefix:'$'})`；非 overview 聚合|API 一期 referrals（金钱）|POST /team/referrals|✅ 已对齐|设计取舍（缺数显0）|—|—|—|R2：要独立🔍（≠CM-011 `direct_presale_volume`）；标签债仍 CM-020；FE 接入证；Prod 对账非本矩阵门闩|
...
|GN-010|共建|Dock·清单|「将获得 AGX」估算|读|`estimateAgxFromUsd1` 已接线展示；Prod 无活期阶段未对拍购后记账|展示估算（非链 getter）|—（纯链）|✅ 已对齐|—|—|—|—|接线在；金额 L 未做；FE 接入证；Prod 对账非本矩阵门闩|
...
|GN-012|共建|Dock·清单|「将获得 X 初始空投价值」|读|`usePresalePreviewAirdropValueQuery`→`addedAirdropValue`|手册 §6 `previewAirdropValue`|—（纯链）|✅ 已对齐|设计取舍（缺数显0）|—|—|—|接线在；0→`$0`；金额 L 未做（旁系 GN-010）；FE 接入证；Prod 对账非本矩阵门闩|
...
|GN-014|共建|Dock·空投门槛数值|`AIRDROP_THRESHOLD`|读|`readPresaleAirdropThresholdWei`→`presaleAirdropThresholdToUsd`；无硬编码|ABI `AIRDROP_THRESHOLD`|—（纯链）|✅ 已对齐|设计取舍（缺数显0）|—|—|—|接线 live；前版「Prod cast/L」无本轮证据→降级；FE 接入证；Prod 对账非本矩阵门闩|
|GN-015|共建|Detail·全球卡|`totalPurchasedAmount`|读|`usePresaleTotalPurchasedQuery`←`readTotalPresalePurchased`；loading Skeleton|手册 §6.3|—（纯链）|✅ 已对齐|设计取舍（缺数显0）|—|—|—|接线在；前版「Prod=0」无本轮证据→降级；FE 接入证；Prod 对账非本矩阵门闩|
|GN-016|共建|Detail·进度头|「本期共建」当前/上限|读|`userPhaseAmountCurrent` / `seasonContributionMaxWei`（limit>0?limit:maxAmount）|手册 `getUserPhaseRemainingAmount`|—（纯链）|✅ 已对齐|设计取舍（缺数显0）|—|—|—|接线在；须钱包；root 见 GN-037；金钱 L 未做；FE 接入证；Prod 对账非本矩阵门闩|
|GN-017|共建|Detail·页脚|「累计共建」`userTotalAmount`|读|`usePresaleUserTotalQuery`←`readUserPresaleTotal`(migration root)→footer `$`|手册 `userTotalAmount(root)`|—（纯链）|✅ 已对齐|设计取舍（缺数显0）|—|—|—|接线在；金钱 L 未做；FE 接入证；Prod 对账非本矩阵门闩|
|GN-018|共建|Detail·贡献表|销售日志分页行（含 `$` amount）|读|`useSalesLogs`+`sessionReady`；`mapSalesLogToDesktopRow` 展 `amount` 为 `$`；未登录 Auth 槽|API 一期 sales|POST /sales/logs|✅ 已对齐|设计取舍（缺数显0）|—|—|—|门闩✅；行内金钱 L 未做（同 CM-011）；FE 接入证；Prod 对账非本矩阵门闩|
...
|GN-022|共建|FAQ|`{phaseCount}`/`{discounts}`/`{phaseQuotas}`/`{minUsd}`/`{shareIncrement}`/`{airdropRatios}`/`{threshold}`|读·Copy|`genesisFaqTemplateValues` 填入 FAQ（`threshold`←同 GN-014；`minUsd`/`phaseQuotas`←phases）|手册 §6 phases|—（纯链）|✅ 已对齐|设计取舍（缺数显0）|—|← GN-014|—|接线在；插值金钱无 Prod→跟 GN-014 降级；`phaseDurationDays`→Z-016；FE 接入证；Prod 对账非本矩阵门闩|
