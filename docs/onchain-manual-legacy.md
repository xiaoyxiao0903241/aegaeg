...
>
> **地址 SSOT**：部署地址以 [`onchain-manual/00-addresses.md`](./onchain-manual/00-addresses.md) 为准；运行时经 `VITE_BSC_*` 注入 `src/shared/config/contracts.ts`（**fail-closed，无代码兜底**）。全量 env 模板：`env/manual.bsc.addresses.env`。新功能对接读 [`onchain-manual/`](./onchain-manual/)。索引见 [`README.md`](./README.md)。
...
console.log(
  '新增-单账户限额(%):',
  phase.userPurchaseLimit == 0n ? '不限制' : ethers.formatEther(phase.userPurchaseLimit),
)
```
...
const quotedAirdropValue = await preSale.quotePhaseAirdropValue(phaseIndex, simulatedPhaseAmount)
...
console.log(
  '新增-该用户剩余可购:',
  userPurchaseLimit === 0n ? '不限制' : ethers.formatEther(remainingUserAmount) + ' USD',
)
