...
| `BondingCalculator` | `0xf661D59D23d136fBFa359AC0dE43d04a778e360C` | 普通地址 |
...
1. 前置检查： Referral.isBindReferral(user)=true ； BondHelper.authContracts(bond)=true ；USD1/AGX Pair 存在；债券容量未满。
2. 授权： USD1.approve(BondHelper, amount) （可一次性 approve 全部债券的总金额，如 script/buy-bonds.ts 的做法）。
3. 买 LP Bond： BondHelper.zapIntoLiquidityBond(bondDepository, USD1, amount) 。
4. 买 Burn Bond： BondHelper.zapIntoBurnBond(burnBondDepository, USD1, amount) 。

**BondHelper 内部（zap）**：

| 债券 | zap 内部流程 |
| --- | --- |
| LP Bond | 一半 USD1 换 AGX（`router.swapExactTokensForTokensSupportingFeeOnTransferTokens`，含 0.25% 手续费 + 滑点）→ 剩下一半 USD1 + 换来的 AGX 组 LP（`router.addLiquidity`）→ 把 LP 转入 `BondDepository.deposit(LP, user)`，多余的 AGX/USD1 退回用户 |
| Burn Bond | 全部 USD1 换 AGX → 把 AGX 转入 `BurnBondDepository.deposit(AGX, user)`，多余 AGX 退回用户 |

**`BondDepository.deposit(amount, depositor)` 内部**（[BondDepository.sol:442-504](../src/BondDepository.sol#L442-L504)）：

1. 迁移检查： migratedTo[msg.sender] / migratedTo[depositor] 任一已迁移 → 回滚。
2. 调用者白名单（启用时）：有 code 的调用者不在 allowedCallers → ErrorCallerNotAllowed 。
3. 推荐检查： referral.isBindReferral(depositor) 否则 → ErrorNotApproved 。
4. 定价： value = Treasury.valueOf(principle, amount) ； payout = value×1e9/agxPrice×10000/discountRateBP 。
5. 边界： ErrorBondTooSmall （<0.01 AGX）/ ErrorBondTooLarge （>maxPayout）/ ErrorDebtCapacityReached （超出 maxDebt）。
...
预估必须与合约实际计算一致，否则会出现"预估 1000 AGX、实际 800 AGX"的偏差。两条债券的定价公式不同（LP Bond 依赖池子价格 + LP 估值；Burn Bond 只依赖换币输出 + 折扣）。前端以**方法二（前端计算）** 作为标准实现；**方法一（eth_call 模拟）** 用于上链前的最精确校验。

**核心公式（与合约源码一致）**：

| 债券 | 输入 | 预估 payout（AGX, 9 位） |
| --- | --- | --- |
| LP Bond 直接存 LP | `lpAmount` | `value * 1e9 / agxPrice * 10000 / discountRateBP` |
| LP Bond 经 zap 存 USD1 | `usd1Amount` | 先模拟换币 + 组 LP，再按上式计算 |
| Burn Bond 直接存 AGX | `agxAmount` | `agxAmount * 10000 / discountRateBP` |
| Burn Bond 经 zap 存 USD1 | `usd1Amount` | `getAmountsOut(usd1, [USD1, AGX])[1] * 10000 / discountRateBP` |

其中：

- value = Treasury.valueOf(principle, amount) 。LP Bond 的 principle 是 AGX/USD1 LP 代币，value 由 AegisLpBondingCalculator.valuation(pair, lpAmount) 给出；Burn Bond 的 principle 是 AGX， valueOf(AGX, amount) 直接返回 amount。
- agxPrice = LP 池 reserveU / reserveAGX （与 BondDepository._getAgxPrice() 一致： token0 == AGX 时 reserveU = reserve1, reserveAGX = reserve0 ，否则相反）。
- discountRateBP = bond.discountRateBP() ，8500 = 付市场价 85%（多拿约 17.6% AGX）。
- 实际到账 netPayout = payout − payout × terms.fee / 10000 （fee 转 DAO）。

##### 方法一：eth_call 模拟（精确校验）

以用户地址模拟真实 zap 交易，直接读取返回的 payout。它跑的是合约真实逻辑，天然包含换币滑点、组 LP、fee 与容量检查，不会与最终交易产生公式偏差，适合提交交易前做最终校验：

js
```js
// 预估 LP Bond（USD1 输入）
const data = bondHelper.interface.encodeFunctionData('zapIntoLiquidityBond', [bondDepoAddr, usd1Addr, usd1Amount]);
const payout = await provider.call({ from: user, to: bondHelperAddr, data });
// 预估 Burn Bond（USD1 输入）
const data2 = bondHelper.interface.encodeFunctionData('zapIntoBurnBond', [burnBondAddr, usd1Addr, usd1Amount]);
const payout2 = await provider.call({ from: user, to: bondHelperAddr, data: data2 });
```

前提：用户需先 `approve` 足够额度（`eth_call` 会模拟 `transferFrom`）。这就是 `script/buy-bonds.ts` 的 `BOND_DRY_RUN` 模式实现。

##### 方法二：前端计算（标准做法，无需授权）

按 BondHelper zap 的内部逻辑在链下复算（与合约一致），不依赖授权状态：

js
```js
// ① 换币：helper 把一半 USD1 换成 AGX（用 router.getAmountsOut 精确含手续费 + 滑点）
const amountToSwap = usd1Amount / 2n;
const agxOut = (await router.getAmountsOut(amountToSwap, [usd1Addr, agxAddr]))[1];
// ② 组 LP：Uniswap V2 公式（min 两侧）
const [r0, r1] = await pair.getReserves();
const totalSupply = await pair.totalSupply();
const [reserveU, reserveAGX] = token0IsAgx ? [r1, r0] : [r0, r1];
const lpFromAgx = agxOut * totalSupply / reserveAGX;
const lpFromUsd = (usd1Amount - amountToSwap) * totalSupply / reserveU;
...
Burn Bond 更简单（无需组 LP）：

js
```js
const agxOut = (await router.getAmountsOut(usd1Amount, [usd1Addr, agxAddr]))[1];
const payout = agxOut * 10000n / discountRateBP;
const netPayout = payout - payout * fee / 10000n;
```

> 合约当前未内置 `estimateLPInfo`/`payoutFor` 这类预估 view，前端按上述 JS 复算即可；若未来需要链上只读预估，可再把换币 + 组 LP 逻辑封装成 `BondHelper.estimateLPInfo(amountIn, principle)` view（返回 `estimatedLP / usdtForSwap / estimatedOhm / usdtForLiquidity / ohmForLiquidity`），前端再套 payout 公式。

##### 输入 → 预估 AGX 示例（主网 2026-08 池子状态：AGX≈55 USDT，discount 8500/8000/7500，fee=0）

| 输入 USD1 | LP Bond 180/360/540d | Burn Bond 180/360/540d |
| --- | --- | --- |
| 1000 | ≈21.3 / 22.6 / 24.1 AGX | ≈21.3 / 22.7 / 24.2 AGX |
| 10000 | ≈213 / 226 / 241 AGX | ≈213 / 227 / 242 AGX |

> 示例基于固定池子状态；实际以 `eth_call` 或实时 view 结果为准。AGX 价格与 `discountRateBP` 变化会直接改变输出；LP Bond 经 zap 会略少于 Burn Bond（多一次换币 + 组 LP 摩擦）。

##### 边界检查

预估结果还要对照合约限制，超出时按钮应禁用并提示：

- payout < 10000000 （0.01 AGX）→ ErrorBondTooSmall
- payout > maxPayout() （ AGX.totalSupply() * terms.maxPayout / 100000 ）→ ErrorBondTooLarge
- terms.totalDeposit + netPayout > terms.maxDebt → ErrorDebtCapacityReached
- 推荐未绑定 → ErrorNotApproved
