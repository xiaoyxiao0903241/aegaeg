...
| **git pre-commit**（husky）                  | staged → eslint + prettier；全仓 → `tsc -b` + **`lint:deadcode`（knip）** + **`lint:duplicates`（jscpd）**。**error 阻断提交**；warn 不阻断。                                         |
...
| `pnpm lint:duplicates`                       | jscpd 拷贝粘贴检测（`.jscpd.json`：`minLines` 15 / `threshold` 0；**进** `check`；import-sort 后抬高以免纯 import 头误报）                                                            |
