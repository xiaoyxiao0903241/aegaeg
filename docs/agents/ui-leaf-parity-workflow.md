...
| **已抽且一眼同构的块仍逐 leaf 深钻 / 为本页再造分叉**（如运行机制）                                 | **§8.2a**：标 `reuse:<Component>`；独有 chrome 才全 leaf                                                              |
...
#### 8.2a 已抽组件 · 一眼同构 → 复用不深钻（用户锁定 2026-08-03）

> **意图：** 本页若与他页差异大，**仍须**对本页独有 chrome 做全最小 leaf（A4）。但对**已经抽取、且稿面一眼与他页同构**的块，**禁止**再逐 leaf 深钻、禁止为本页再造分叉实现。

| 条件（须同时）                                                                                            | 动作                                                                                           |
| --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 该块已有共用组件（如 `DappProcessSteps` / `MetricGrid` / `StakingChartCard` / `FaqList` / `TvAreaChart`） | **直接复用** call site；leaf 标 `reuse:<Component>`                                            |
| 稿面与已贴齐页（如 Stake / Bond）**一眼同构**（同区块角色、同 chrome 骨架；文案/数字可 diff）             | **不**对该块内每个 text/icon 再计独立 A4 深钻；**不**要求本页再跑该块全量 `get_design_context` |
| 任一条不成立（未抽取 / 结构明显不同 / 本页独有控件）                                                      | **仍须**全 leaf 深钻                                                                           |

**计入 meta_min_leaf_count：** 复用块整块可记 **1 行**（父 surface + `reuse:`），其子 text/icon **不**强制计入 N；独有 chrome 仍按 §2.3a 逐叶计入。

**禁止：** 用本条跳过本页**独有**左栏/表单/仓位布局；把「文案不同」当成「结构不同」而重造已抽组件；未抽取却自称 reuse。
