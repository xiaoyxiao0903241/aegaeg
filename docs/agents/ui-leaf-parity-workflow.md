...
> **状态：** 用户锁定 2026-07-30（对抗纠偏：堵 R5 合取漏洞 · 手册沉默 ≠ 取消 UI · WebBridge 实录字段）；**2026-07-31 加锁：手册相关章必须逐行阅读 + leaf 逐条对照表**  
...
2. **逐条对照表**：手册每一行（展示字段 / 写方法 / 前置检查 / 成功后刷新 / 注意事项）→ 现码路径或「缺口/产品扩展」；**禁止**用「整体符合 §X」冒充。
...
- [ ] 手册 **逐行**已读且 leaf 有对照表；有源接线；无源=`—`且 **UI 仍在**
