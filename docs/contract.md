...
>
> **本仓库实现入口（行为 SSOT）**：写链经 `src/views/dapp/web3/wallet-contract-write.ts`；绑定 `referral-write.ts`；预售 `presale-write.ts`；奖励领取 `reward-claim.ts`（API 签名 → `normalize-claim-payload.ts` → 链上 `claimReward`）。地址 SSOT：`src/shared/config/contracts.ts`。
