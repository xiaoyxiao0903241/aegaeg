...
>
> **本仓库实现入口（行为 SSOT）**：写链经 `src/web3/wallet/wallet-contract-write.ts`；绑定 `src/web3/referral-write.ts`；预售 `src/web3/presale-write.ts`；奖励领取 `src/web3/reward-claim.ts`（API 签名 → `normalize-team-reward-claim.ts` → 链上 `claimReward`）。地址 SSOT：`src/shared/config/contracts.ts`。目录落点见 [`src-layout.md`](./src-layout.md)。
