...
7. **页专属 hook** → 与页袋同目录（`views/dapp/<tab>/use-*.ts`）
...
src/
  ...
  views/
    ...
    dapp/
      web3/                       # 链网关 + 读适配（use-chain-read-client、use-presale-queries）
...
| 袋 | hook |
|----|------|
| `views/dapp/swap/` | `use-swap-widget` · `use-flash-swap-widget` · `use-swap-quote` · `use-swap-pool-reads` |
...
| `views/dapp/web3/` | `use-chain-read-client` · `use-presale-queries` |
...
- 整袋 `views/dapp/web3` → `src/web3` 上提（须单独对抗；先保持 `home-no-web3`）
- god hook 按「读 / 写 / 提交」内部分拆
