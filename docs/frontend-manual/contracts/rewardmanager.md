...
- 按 ppm 给 StakingPool 和其他普通 recipient 铸币（普通 recipient 按 supply * rate / 1,000,000 ，源码 src/RewardManager.sol:249 ）。
- 按基础 Rebase 的百分比给 DaoPool 补充奖励储备。
...
| `DAO_REWARD_RATIO`     | %    | `168`    | DAO 储备预算：`baseReward × ratio / 100`                 |
...
白皮书明确的是每 12 小时基础 Rebase 0.25%—0.5%，以及 180/360/540 天池至少 10%/15%/20% 的“基础收益加成”。`DAO_REWARD_RATIO=168` 是按当前 scanner 奖励公式理论上限设置的偿付预算，不是白皮书固定比例。
...
- previewEpochRewards() ：返回下一轮基础奖励、DAO、普通 recipient、长期池以及总资金需求。
...
| `setDaoRewardRatio(uint256 _newRatio)`                                     | `onlyOwner`      | 更新 DAO 奖励比例，要求 `_newRatio <= 500`。源码 `:375`                               |
