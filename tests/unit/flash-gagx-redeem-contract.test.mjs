...
test('flash intros include wrap direction and dual-pair copy', async () => {
  ...
  assert.ok(flash.gates.insufficientOutput)
  assert.ok(flash.gates.transferMismatch)
  assert.ok(flash.gates.zeroAddress)
  assert.ok(flash.gates.sameToken)
  assert.ok(flash.gates.zeroAmount)
  assert.ok(flash.gates.notAuthorized)
  assert.ok(flash.gates.invalidLimits)
