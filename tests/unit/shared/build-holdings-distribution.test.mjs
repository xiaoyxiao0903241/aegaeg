...
test('buildHoldingsDistributionView percentages and empty', async () => {
  ...
  assert.ok(view)
  assert.equal(view.segs[0].pctLabel, '52.90%')
  assert.equal(view.segs[1].pctLabel, '30.52%')
  assert.equal(view.segs[2].pctLabel, '15.26%')
  assert.equal(view.segs[3].pctLabel, '1.32%')
