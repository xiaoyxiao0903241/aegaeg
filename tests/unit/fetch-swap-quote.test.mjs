...
test('fetchSwapQuote wires pool fee, quoter out, and price impact bps', async () => {
  const { fetchSwapQuote } = await loadModule('/src/views/dapp/web3/swap-read.ts')
  const { clearSwapPoolImmutableCache } = await loadModule('/src/views/dapp/web3/read-swap-pool.ts')
  ...
  assert.equal(result.priceImpactBps, 100)
...
test('quoteV3ExactInputSingle returns zeros for zero amountIn without RPC', async () => {
  const { quoteV3ExactInputSingle } = await loadModule('/src/views/dapp/web3/quote-v3-exact-input.ts')
  const client = createMockClient()
