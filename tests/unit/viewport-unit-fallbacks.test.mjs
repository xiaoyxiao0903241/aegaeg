...
test('injectViewportUnitFallbacks: rewrites compound values and Tailwind utilities', () => {
  ...
  assert.match(out, /\.x\{max-height:min\(92vh,100vh - 2rem\);max-height:min\(92dvh,100dvh - 2rem\)\}/)
...
test('source stays dvh-only — Chrome90 fallback is build-time, not hand-written', () => {
  ...
  const shell = readFileSync(join(root, 'src/app/dapp-shell.tsx'), 'utf8')
  ...
  assert.doesNotMatch(lock, /100vh/, 'do not hand-write vh in source; use viewport-unit-fallbacks plugin')
