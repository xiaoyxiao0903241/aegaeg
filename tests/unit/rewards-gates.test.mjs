...
import {
  evaluateRewardsMixedClaimGate,
  evaluateRewardsSimpleClaimGate,
  isLuckyClaimable,
} from '../../src/core/rewards/rewards-gates.ts'

test('simple claim separates session gate from zero amount', () => {
  assert.equal(
    evaluateRewardsSimpleClaimGate({ sessionReady: false, amount: 0n, unknownLocked: false }),
    'notSessionReady',
  )
  assert.equal(
    evaluateRewardsSimpleClaimGate({ sessionReady: true, amount: 0n, unknownLocked: false }),
    'zeroAmount',
  )
  assert.equal(
    evaluateRewardsSimpleClaimGate({ sessionReady: true, amount: 1n, unknownLocked: true }),
    'lockedUnknown',
  )
  assert.equal(
    evaluateRewardsSimpleClaimGate({ sessionReady: true, amount: 1n, unknownLocked: false }),
    null,
  )
})
