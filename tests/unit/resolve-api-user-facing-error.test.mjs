...
test('resolveApiUserFacingError maps transport codes and never returns raw message', async () => {
  const { ApiError } = await loadModule('/src/shared/api/client.ts')
  const { API_TRANSPORT_ERROR, resolveApiUserFacingError } = await loadModule(
    '/src/shared/api/resolve-api-user-facing-error.ts',
  )
  ...
  assert.equal(resolveApiUserFacingError(new Error('execution reverted'), messages), null)
})
...
test('apiRequest wraps network failures as NETWORK ApiError', async () => {
  ...
  const { API_TRANSPORT_ERROR } = await loadModule(
    '/src/shared/api/resolve-api-user-facing-error.ts',
  )
