...
test('confirmClaimWithRetry succeeds after transient failures', async () => {
  ...
  globalThis.fetch = async () => {
    calls += 1
    return Response.json({ code: 401, error: 'UNAUTHORIZED', message: 'expired' }, { status: 401 })
  }
  try {
...
test('evaluateGenesisPostApproveGate blocks unbound / paused', async () => {
  const { evaluateGenesisPostApproveGate } = await loadModule('/src/core/presale/presale-math.ts')
