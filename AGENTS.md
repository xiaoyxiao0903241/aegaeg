...
- **`shared/components`**：只扩**无业务数据**的布局/控件 primitive（`Tile`/`Grid`/`MainButton`…）；业务档位 / locale / 地址不进。跨 tab **产品壳**（读 store / 绑钱包 / i18n 默认，如 `DockFrame`）进 `views/dapp/shared/`（非 tab）；不确定宁放页袋。详见 [`docs/decisions/dapp-page-bag-dock-detail.md`](docs/decisions/dapp-page-bag-dock-detail.md)。
