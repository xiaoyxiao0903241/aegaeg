...
export function HomeMetricsSection() {
  ...
  return (
    <HomeSection
      container="content"
      className="dapp:min-h-56 pb-10 max-dapp:min-h-64 max-dapp:pb-12"
    ...
    >
      <div
        className="relative isolate grid min-h-48 grid-cols-4 items-center justify-between rounded-xl bg-dark px-10 py-14 text-white max-dapp:min-h-52 max-dapp:grid-cols-2 max-dapp:gap-y-6 max-dapp:rounded-lg max-dapp:px-5 max-dapp:py-7 after:pointer-events-none after:absolute after:-top-6.5 after:left-1/2 after:z-0 after:h-16 after:w-16 after:-translate-x-1/2 after:rounded-full after:bg-white/56 after:blur-[1.125rem] after:content-[''] max-dapp:after:hidden"
        data-metrics-panel
      >
        <div
        ...
        {metrics.map((metric) => (
          ...
          >
            ...
            <Text
              as="span"
              className="text-sm font-medium leading-[1.2] text-white max-dapp:text-xs max-dapp:font-normal max-dapp:leading-[1.5] max-dapp:text-white/70"
            >
              {metric.label}
            </Text>
          </article>
