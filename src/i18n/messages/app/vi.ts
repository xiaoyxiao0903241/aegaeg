import { defineMessages } from '~/i18n/messages/define-messages'
import type { AppMessagesBundle } from './types'

const app = defineMessages({

  common: {
    brand: 'AEGIS X',
    bsc: 'BSC',
    connectWallet: 'Kết nối ví',
    language: 'Ngôn ngữ',
    copy: 'Sao chép',
    success: 'Thành công',
    paid: 'Đã thanh toán',
    claimable: 'Chờ nhận',
    current: 'Hiện tại',
    max: 'Tối đa',
    collapse: 'Thu gọn',
    expand: 'Mở rộng',
    confirm: 'Xác nhận',
    close: 'Đóng',
    paginationTotal: 'Tổng {total} mục',
    paginationPerPage: '{size} mục / trang',
    paginationPrev: 'Trang trước',
    paginationNext: 'Trang sau',
  },
  nav: {
    swap: 'Swap',
    genesis: 'Cùng xây dựng',
    rewards: 'Phần thưởng',
    community: 'Cộng đồng',
    genesisTooltip: 'Tham gia mùa đăng ký Genesis và nhận danh hiệu cổ đông.',
    rewardsTooltip: 'Xem phần thưởng giới thiệu và phần thưởng nhóm.',
    communityTooltip: 'Mời bạn bè, liên kết người giới thiệu và phát triển nhóm.',
    bscTooltip:
      'Chỉ BSC · AEGIS X chạy trên BNB Smart Chain, chưa hỗ trợ chuyển mạng.',
  },
  topbar: {
    currentNetwork: 'Mạng hiện tại',
    openMenu: 'Mở điều hướng',
    closeMenu: 'Đóng điều hướng',
    hideDetails: 'Thu gọn bảng chi tiết',
    showDetails: 'Mở rộng bảng chi tiết',
    toggleTooltip: 'Hiện hoặc ẩn bảng chi tiết',
  },
  wallet: {
    connectTitle: 'Kết nối ví',
    connectIntroTitle: 'Kết nối ví để khám phá các tính năng AEGIS X',
    connectIntroLink: 'Tính năng AEGIS X ↗',
    installedWallets: 'Ví đã cài đặt',
    suggestedWallets: 'Ví đề xuất',
    connectAction: 'Kết nối',
    connecting: 'Đang kết nối…',
    termsPrefix: 'Kết nối đồng nghĩa với việc bạn đồng ý với',
    termsLink: 'Điều khoản dịch vụ',
    viewAllWallets: 'Xem tất cả ví',
    allWalletsTitle: 'Tất cả ví',
    scanQrInstruction: 'Quét mã QR này bằng ứng dụng ví để kết nối.',
    copyQrLink: 'Sao chép liên kết',
    qrLinkCopied: 'Đã sao chép',
    connectingWallet: 'Đang chờ xác nhận từ ví…',
    openWalletApp: 'Đang mở ứng dụng ví…',
    connectionFailed: 'Kết nối thất bại',
    tryAgain: 'Thử lại',
    supportedNetwork: 'Mạng được hỗ trợ',
    copyAddress: 'Sao chép địa chỉ',
    copied: 'Đã sao chép',
    disconnect: 'Ngắt kết nối',
    reconnectWallet: 'Kết nối lại ví',
    reconnectHint: 'Ví đã ngắt kết nối. Vui lòng kết nối lại trước khi thực hiện thao tác on-chain.',
    signInRequired: 'Đăng nhập bằng chữ ký',
  },
  swap: {
    title: 'Swap',
    intro: 'Tỷ giá thị trường PancakeSwap theo thời gian thực, thanh toán on-chain trong vài giây.',
    disconnectedIntro: 'USDT ↔ USD1 · cố định 1:1 · không trượt giá.',
    sell: 'Sell',
    buy: 'Buy',
    flip: 'Đổi chiều hoán đổi',
    balance: 'Số dư',
    rate: 'Tỷ giá hoán đổi',
    exchangePrice: 'Giá hoán đổi',
    ratePlaceholder: '1 : 1',
    slippage: 'Dung sai trượt giá',
    allowedSlippage: 'Trượt giá cho phép',
    slippageSettings: 'Cài đặt dung sai trượt giá',
    route: 'Lộ trình hoán đổi',
    provider: 'Nhà cung cấp',
    providerName: 'PancakeSwap',
    openPancakeSwap: 'Mở trên PancakeSwap',
    approve: 'Ủy quyền',
    action: 'Swap',
    approveSuccess: 'Ủy quyền thành công',
    swapSuccess: 'Hoán đổi thành công',
    connectToSwap: 'Kết nối ví để hoán đổi',
    connectExploreTitle: 'Kết nối để khám phá các tính năng AEGIS X',
    connectExploreAction: 'Các tính năng có thể dùng sau khi kết nối →',
    overview: 'Tổng quan',
    exchangeRate: 'Tỷ giá hoán đổi',
    fixedRate: '1 : 1 cố định',
    fixedRateConnected: '1 : 1.001',
    fixedRateHint: 'USDT ↔ USD1 · giao thức không chênh lệch',
    settlement: 'Thanh toán',
    settlementValue: 'PancakeSwap · nhận ngay',
    settlementHint: 'Chỉ BSC · phí gas do ví đã kết nối trả',
    about: {
      title: 'Giải thích',
      body: 'USDT và USD1 được hoán đổi ngay lập tức theo tỷ giá cố định 1:1, không trượt giá. USD1 là tài sản thanh toán cốt lõi của giao thức AEGIS X.',
    },
    tokenAbout: {
      title: 'Về token hệ sinh thái AEGIS X',
      items: [
        {
          key: 'usd1',
          title: 'USD1 · Tài sản thanh toán cốt lõi',
          body: 'Tài sản thanh toán cốt lõi của hệ sinh thái AEGIS X, kết nối lưu thông giá trị, mạng thanh khoản và kịch bản thanh toán.',
        },
        {
          key: 'agx',
          title: 'AGX · Tài sản giao thức cốt lõi',
          body: 'AGX là tài sản cốt lõi của giao thức AEGIS X, được tạo ra thông qua cơ chế thế chấp vượt mức 150%, đảm nhiệm vai trò quan trọng trong tăng trưởng giá trị, phân phối lợi nhuận và xây dựng hệ sinh thái.',
          bodyDesktop:
            'Token quản trị và giá trị cốt lõi của giao thức AEGIS X, dùng cho bỏ phiếu quản trị, staking và khuyến khích hệ sinh thái, mang giá trị tăng trưởng dài hạn của giao thức.',
        },
        {
          key: 'gagx',
          title: 'gAGX · Chứng nhận thanh toán phần thưởng',
          body: 'Chứng nhận thanh toán phần thưởng giao thức, có thể đổi lấy AGX và tham gia khai thác hệ sinh thái cùng tái tuần hoàn lợi nhuận.',
          bodyDesktop: 'Chứng nhận sinh lãi nhận được khi staking AGX, tự động lãi kép, mở khóa trọng số quản trị và danh hiệu cao hơn.',
        },
        {
          key: 'x',
          title: 'X · Token giá trị hệ sinh thái',
          body: 'Phương tiện mang giá trị hệ sinh thái AEGIS X, nguồn cung cố định 210 triệu, mang tăng trưởng hệ sinh thái và tích lũy giá trị.',
          bodyDesktop: 'Token tham gia và quyền lợi hệ sinh thái, ghi nhận đóng góp on-chain, có thể đổi quyền lợi, tham gia sự kiện và nhận hệ số airdrop.',
        },
      ],
    },
    tokenContract: 'Xem hợp đồng',
    tokenPrevious: 'Token trước',
    tokenNext: 'Token tiếp theo',
    swipeNext: 'Vuốt lên xem trang tiếp theo',
    swipePrevious: 'Vuốt xuống quay lại trang trước',
    faq: {
      title: 'FAQ',
      tabs: {
        usd1: {
          label: 'USD1',
          items: [
            {
              q: 'USD1 là gì?',
              a: 'USD1 là tài sản thanh toán giá trị cốt lõi của hệ sinh thái AEGIS X. USD1 được hỗ trợ 100% bởi tài sản dự trữ, bao gồm tiền mặt, trái phiếu kho bạc Mỹ ngắn hạn, quỹ thị trường tiền tệ chính phủ, v.v. Báo cáo phân bổ số tiền có thể xem hàng tháng trên trang web chính thức WLFI.',
            },
            {
              q: 'USD1 đảm nhiệm vai trò gì trong AEGIS X?',
              a: 'Là tài sản thanh toán cốt lõi, USD1 kết nối mạng thanh khoản, kịch bản thanh toán và lưu thông giá trị hệ sinh thái.',
            },
            {
              q: 'Làm thế nào để hoán đổi USD1?',
              a: 'Người dùng có thể nhanh chóng hoán đổi USDT thành USD1 thông qua tính năng hoán đổi on-chain để tham gia hệ sinh thái AEGIS X.',
            },
          ],
        },
        agx: {
          label: 'AGX',
          items: [
            {
              q: 'AGX là gì?',
              a: 'AGX là token quản trị và giá trị của AEGIS X, dùng cho bỏ phiếu quản trị, staking và khuyến khích hệ sinh thái.',
            },
            {
              q: 'USD1 đóng vai trò gì trong giao thức?',
              a: 'USD1 là kênh thanh toán xuyên suốt cho đăng ký, phần thưởng và hoán đổi, giúp ghi nhận giá trị đơn giản và có thể kiểm toán.',
            },
            {
              q: 'USDT và USD1 được hoán đổi như thế nào?',
              a: 'Ứng dụng hoàn tất chuyển đổi qua lộ trình hoán đổi BSC được hỗ trợ, giao dịch cuối cùng do ví đã kết nối xác nhận.',
            },
          ],
        },
        gagx: {
          label: 'gAGX',
          items: [
            {
              q: 'gAGX là gì?',
              a: 'gAGX là chứng nhận sinh lãi nhận được khi staking AGX, mở khóa trọng số quản trị và danh hiệu cổ đông cao hơn.',
            },
            {
              q: 'USD1 đóng vai trò gì trong giao thức?',
              a: 'USD1 là kênh thanh toán xuyên suốt cho đăng ký, phần thưởng và hoán đổi, giúp ghi nhận giá trị đơn giản và có thể kiểm toán.',
            },
            {
              q: 'USDT và USD1 được hoán đổi như thế nào?',
              a: 'Ứng dụng hoàn tất chuyển đổi qua lộ trình hoán đổi BSC được hỗ trợ, giao dịch cuối cùng do ví đã kết nối xác nhận.',
            },
          ],
        },
        x: {
          label: 'X',
          items: [
            {
              q: 'X là gì?',
              a: 'X là token quyền lợi tham gia hệ sinh thái, ghi nhận đóng góp on-chain và đủ điều kiện nhận airdrop cùng hệ số sự kiện.',
            },
            {
              q: 'USD1 đóng vai trò gì trong giao thức?',
              a: 'USD1 là kênh thanh toán xuyên suốt cho đăng ký, phần thưởng và hoán đổi, giúp ghi nhận giá trị đơn giản và có thể kiểm toán.',
            },
            {
              q: 'USDT và USD1 được hoán đổi như thế nào?',
              a: 'Ứng dụng hoàn tất chuyển đổi qua lộ trình hoán đổi BSC được hỗ trợ, giao dịch cuối cùng do ví đã kết nối xác nhận.',
            },
          ],
        },
      },
    },
    tokenContractTooltip: 'Xem chi tiết token và hợp đồng',
  },
  genesis: {
    title: 'Kế hoạch cùng xây dựng',
    intro: 'Tham gia kế hoạch cùng xây dựng X DAO · Kỳ {season} ({discount} giảm giá)',
    shares: 'Phần (1 phần = 100 USD1 · tối đa 100)',
    hint: 'Kỳ này còn có thể đăng ký 32 phần · Toàn mạng còn lại 40 phần',
    quota: 'Hạn mức cùng xây dựng kỳ này',
    pay: 'Thanh toán',
    receive: 'Sẽ nhận AGX',
    value: 'Giá trị đăng ký',
    xTokenAirdrop: 'Giá trị airdrop X ban đầu dự kiến',
    xTokenAirdropHint:
      'Phần thưởng airdrop yêu cầu tổng tham gia đồng xây ≥ 5.000 USD mỗi tài khoản mỗi mùa.',
    join: 'Tham gia cùng xây dựng',
    joinGenesis: 'Join Genesis',
    statsTitle: 'Dữ liệu cùng xây dựng kỳ {season}',
    startsIn: 'Đếm ngược bắt đầu',
    endsIn: 'Thời gian còn lại kỳ này',
    referencePrice: 'Giá tham chiếu mở cửa AGX',
    contribution: 'Đăng ký toàn mạng',
    discountLabel: 'Giảm giá',
    discountRatio: 'Tỷ lệ giảm giá kỳ này',
    network: 'Toàn mạng',
    xAirdropRatio: 'Tỷ lệ airdrop X',
    airdropLabel: 'Tỷ lệ airdrop X',
    addresses: 'Số địa chỉ',
    participants: 'Người tham gia Genesis',
    seasons: 'Mùa đăng ký',
    myContributions: 'Lịch sử cùng xây dựng của tôi',
    connectBrandLine: 'Bảo vệ mạng giá trị tương lai',
    contributionsConnectTitle: 'Kết nối ví để xem bản ghi của bạn',
    contributionsConnectBody: 'Sau khi kết nối, lịch sử cùng xây dựng sẽ hiển thị tại đây.',
    totalContributed: 'Tổng cùng xây dựng',
    globalLabel: 'Tổng cùng xây dựng toàn cầu',
    globalBody: 'Quy tụ các nhà cùng xây dựng cốt lõi toàn cầu, cùng xây dựng mạng lưới sinh thái toàn cầu AEGISX.',
    viewContract: 'Xem hợp đồng',
    faq: {
      title: 'FAQ',
      items: [
        {
          q: 'Làm thế nào để tham gia kế hoạch cùng xây dựng?',
          a: 'Người dùng dùng USD1 tham gia cùng xây dựng, có thể nhận AGX theo mức giảm giá tương ứng từng giai đoạn. Ba kỳ, mỗi kỳ mở 20 ngày, mức giảm giá lần lượt 30% / 25% / 20%.',
        },
        {
          q: 'Hạn mức cùng xây dựng và yêu cầu tham gia?',
          a: 'Số tiền tham gia tối thiểu 100 USD1, phải tham gia theo bội số 100 USD1. Kỳ 1: $100–$10.000, kỳ 2: $100–$20.000, kỳ 3: $100–$30.000.',
        },
        {
          q: 'Chu kỳ cùng xây dựng kéo dài bao lâu?',
          a: 'AGX nhận được từ tham gia cùng xây dựng áp dụng chu kỳ giải phóng 540 ngày.',
        },
        {
          q: 'Làm thế nào để nhận phần thưởng airdrop X?',
          a: 'Tài khoản tích lũy tham gia cùng xây dựng đạt 5.000 USD sẽ đủ điều kiện nhận phần thưởng airdrop X tương ứng giai đoạn. Tỷ lệ airdrop ba kỳ lần lượt 5% / 2% / 1%.',
        },
        {
          q: 'Phần thưởng airdrop X được giải phóng như thế nào?',
          a: 'Phần thưởng airdrop X áp dụng cơ chế giải phóng tuyến tính 12 tháng, mỗi tháng giải phóng khoảng 8,33%; lần giải phóng đầu tiên là ngày thứ 30 sau khi giao thức staking X ra mắt, tự động thực hiện bởi hợp đồng thông minh.',
        },
      ],
    },
    promoTitle: 'Genesis Season 1 · giảm 30%',
    promoBody: 'Đang diễn ra, hạn mức có hạn, hết hạn 07.10',
    promoTitleTemplate: 'Genesis Season {season} · {discount}',
    promoLive: 'Đang diễn ra, hạn mức có hạn, hết hạn {endDate}',
    promoUpcoming: 'Sắp bắt đầu, hạn mức có hạn, bắt đầu {startDate}',
    promoEnded: '{status} · {date}',
    promoLoading: 'Đang tải thông tin mùa…',
    joinSuccess: 'Đăng ký thành công',
    approved: 'Đã ủy quyền',
    insufficientUsd1: 'Số dư USD1 không đủ. Vui lòng có đủ USD1 trước khi tham gia đăng ký.',
    insufficientAllowance: 'Ủy quyền USD1 không đủ. Vui lòng nhấn ủy quyền trước.',
    purchaseUnavailable: 'Hiện không thể tham gia đăng ký. Vui lòng kiểm tra số phần hoặc trạng thái mùa đăng ký.',
    walletNotConnected: 'Ví đã ngắt kết nối. Vui lòng kết nối lại trước khi ký giao dịch.',
    contributionsSyncPending: 'Đăng ký on-chain đã xác nhận, lịch sử đang đồng bộ, vui lòng làm mới sau.',
    contributionsEmpty: {
      title: 'Chưa có lịch sử cùng xây dựng',
    },
    usd1Balance: 'Giá trị cùng xây dựng',
    phase: 'Sẽ nhận airdrop X',
    phaseSeason: 'Season {season} · {discount}',
    seasonLive: 'Đang diễn ra',
    seasonEnded: 'Đã kết thúc',
    seasonUpcoming: 'Sắp bắt đầu',
  },
  rewards: {
    title: 'Phần thưởng cùng xây dựng',
    intro: 'Tham gia cùng xây dựng · Chia sẻ giá trị tăng trưởng',
    currentTitle: 'Cấp hiện tại',
    shareholder: 'S2 · Cổ đông sáng lập',
    shareholderHint: 'Phần thưởng đăng ký nhóm 2% · Danh hiệu A3 30 ngày sau ra mắt',
    shareholderHintForRank: 'Phần thưởng đăng ký nhóm {bonus} · Danh hiệu {postLaunch} 30 ngày sau ra mắt',
    shareholderHintNoRank: 'Cấp sáng lập',
    shareholderNoRankTitle: 'Bạn chưa trở thành cổ đông sáng lập',
    shareholderNoRankBody:
      'Trở thành cổ đông sáng lập để nhận 1%-10% khối lượng đồng xây dựng của đội làm phần thưởng và nâng 1 cấp trong 30 ngày sau khi AEGIS X ra mắt.',
    shareholderTitleForRank: '{rank} · Cổ đông sáng lập',
    heroKicker: 'Cấp sáng lập',
    heroBody: 'Số tiền đăng ký nhóm có thể nhận phần thưởng {bonus}, và sau ra mắt nhận danh hiệu builder {postLaunch} trong 30 ngày.',
    heroBodyForRank:
      'Số tiền đăng ký nhóm có thể nhận phần thưởng {bonus}, và sau ra mắt nhận danh hiệu builder {postLaunch} trong 30 ngày.',
    currentTierSuffix: 'Hiện tại',
    progressPersonalTo: 'Còn cách {rank} · Đăng ký cá nhân',
    progressTeamTo: 'Còn cách {rank} · Đăng ký nhóm',
    progressMaxPersonal: 'Đã đạt cấp cá nhân cao nhất',
    progressMaxTeam: 'Đã đạt cấp nhóm cao nhất',
    teamLegRequirement: 'Hai nhánh {rank}',
    teamVolume: 'Doanh số hệ thống',
    referralRewards: 'Phần thưởng giới thiệu trực tiếp',
    autoPaidLabel: 'Tự động thanh toán',
    autoPaid: 'Phần thưởng tự động thanh toán vào ví',
    teamRewards: 'Phần thưởng cấp',
    claimed: 'Đã nhận {amount}',
    claim: 'Nhận vào ví',
    heroTitle: 'Cấp hiện tại',
    allTiers: 'Hệ thống danh dự sáng lập',
    history: 'Lịch sử phần thưởng',
    historyConnectBody: 'Sau khi kết nối, lịch sử phần thưởng sẽ hiển thị tại đây.',
    referralHistory: 'Phần thưởng giới thiệu',
    teamHistory: 'Phần thưởng nhóm',
    referralHistoryEmpty: {
      title: 'Chưa có lịch sử phần thưởng giới thiệu',
      body: 'Phần thưởng giới thiệu sẽ hiển thị tại đây sau khi người được giới thiệu hoàn tất đăng ký trong thời gian Genesis.',
    },
    teamHistoryEmpty: {
      title: 'Chưa có lịch sử phần thưởng nhóm',
      body: 'Lịch sử quyết toán và nhận phần thưởng nhóm sẽ hiển thị tại đây khi phần thưởng phát sinh.',
    },
    faq: {
      title: 'FAQ',
      items: [
        {
          q: 'Phần thưởng giới thiệu được tính như thế nào?',
          a: 'Phần thưởng giới thiệu là 3%, áp dụng cơ chế quyết toán nén số tiền tương đương, chỉ tính theo phần số tiền tương đương, tài khoản trống không tính tầng phần thưởng, phần thưởng tự động quyết toán.',
        },
        {
          q: 'Cấp sáng lập thăng tiến như thế nào?',
          a: 'Cấp sáng lập từ S1 đến S10, đánh giá theo số tiền cùng xây dựng cá nhân và tổng doanh số hệ thống, cấp cao cần đáp ứng điều kiện thăng tiến hai khu vực.',
        },
        {
          q: 'Phần thưởng nâng cấp cấp là gì?',
          a: 'Cấp sáng lập đạt được trong thời gian cùng xây dựng sẽ tự động nâng 1 cấp sau khi giao thức ra mắt, có hiệu lực 30 ngày, sau đó trở về cấp thực tế.',
        },
        {
          q: 'Phần thưởng nhóm sáng lập được quyết toán như thế nào?',
          a: 'Phần thưởng nhóm sáng lập tính theo tỷ lệ cấp sáng lập tương ứng, hợp đồng thông minh tự động quyết toán lúc 00:00 (UTC) mỗi ngày, người dùng tự nhận vào ví khi cần. Sau khi kỳ cùng xây dựng kết thúc, trang hiện tại sẽ đóng, phần thưởng chưa nhận không thể nhận nữa, phần thưởng sẽ chuyển vào hợp đồng market maker thông minh.',
        },
      ],
    },
    teamHistorySource: 'Quyết toán chu kỳ',
    rewardType: {
      referralPaid: 'Phần thưởng giới thiệu',
      referralWithdrawn: 'Nhận phần thưởng giới thiệu',
      marketTeam: 'Phần thưởng nhóm market maker',
      presaleTeam: 'Phần thưởng nhóm presale',
      unknown: '—',
    },
    logStatus: {
      pending: 'Chờ xử lý',
      processing: 'Đang xử lý',
      paid: 'Đã thanh toán',
      claimed: 'Đã nhận',
      failed: 'Thất bại',
      unknown: '—',
    },
  },
  community: {
    title: 'Cộng đồng',
    intro: 'Mời đối tác tham gia cùng xây dựng, chia sẻ giá trị tăng trưởng hệ sinh thái và phần thưởng sáng lập.',
    disconnectedIntro: 'Kết nối ví để tạo liên kết giới thiệu và liên kết người mời.',
    referralLink: 'Liên kết mời của tôi',
    shareReferral: 'Sao chép liên kết',
    referrer: 'Người mời của tôi',
    bindReferrer: 'Liên kết',
    referrerPlaceholder: 'Nhập địa chỉ người giới thiệu (0x…)',
    referrerHint: 'Quan hệ mời kích hoạt sẽ có hiệu lực vĩnh viễn, không thể thay đổi.',
    docs: 'Tài liệu',
    twitter: 'Twitter / X',
    telegram: 'Telegram',
    shareholder: 'Tham gia cùng xây dựng',
    myCommunity: 'Cộng đồng của tôi',
    directReferrals: 'Số giới thiệu trực tiếp',
    myTeam: 'Số thành viên cộng đồng',
    genesisTitle: 'Hiện tại',
    inviteTitle: 'Bắt đầu mời · Chia sẻ giá trị tăng trưởng hệ sinh thái',
    programs: {
      title: 'Kế hoạch hỗ trợ hệ sinh thái',
      items: [
        {
          label: 'X DAO cùng xây dựng · Kỳ 1',
          title: 'Kế hoạch cùng xây dựng toàn cầu đang diễn ra',
          body: 'Quy tụ các nhà cùng xây dựng toàn cầu, cùng tham gia xây dựng hệ sinh thái.',
          action: 'Xem chi tiết kế hoạch →',
        },
        {
          label: 'X Academy',
          title: 'Kế hoạch đào tạo hệ sinh thái dành cho nhà cùng xây dựng',
          body: 'Giúp nhà cùng xây dựng hiểu sâu hơn về cơ chế hệ sinh thái và kế hoạch phát triển.',
          action: 'Xem chi tiết kế hoạch →',
        },
      ],
    },
    myInvites: 'Thành viên cộng đồng của tôi ({count})',

    referralBondPermanent: 'Quan hệ giới thiệu đã kích hoạt · Liên kết vĩnh viễn.',
    referralBondActiveSince: 'Quan hệ giới thiệu đã kích hoạt từ {date} · Liên kết vĩnh viễn.',
    volumePrefix: 'Doanh số',
    genesisShareholder: 'Cổ đông Genesis',
    statDirectVolume: 'Doanh số $48,200',

    statTeamVolume: 'Doanh số $412,900',

    statGenesisVolume: 'Cổ đông Genesis',
    statDirectToday: 'Hôm nay +2 · +$1,200',
    statTeamToday: 'Hôm nay +9 · +$8,640',
    statGenesisToday: 'Tự động nâng 1 cấp sau ra mắt',

    bindReferrerSuccess: 'Liên kết người giới thiệu thành công',
    inviteFlow: {
      items: [
        {
          title: 'Chia sẻ liên kết mời',
          body: 'Kết nối ví và điền người mời của bạn để tạo liên kết mời riêng.',
        },
        {
          title: 'Đối tác tham gia cùng xây dựng',
          body: 'Đối tác đăng ký qua liên kết mời của bạn có thể tham gia cùng xây dựng.',
        },
        {
          title: 'Nhận phần thưởng cùng xây dựng',
          body: 'Sau khi đối tác tham gia cùng xây dựng, phần thưởng tự động quyết toán vào địa chỉ ví của bạn bởi hợp đồng thông minh.',
        },
      ],
    },
    boundTo: 'Đã liên kết {address}',
    invitesEmpty: {
      title: 'Chưa có lịch sử mời',
      body: 'Chia sẻ liên kết giới thiệu, mời bạn bè tham gia cộng đồng của bạn.',
    },
    faq: {
      title: 'FAQ',
      items: [
        { q: 'FAQ1', a: '---' },
        { q: 'FAQ2', a: '---' },
      ],
    },
  },
  tables: {
    time: 'Thời gian',
    paid: 'Thanh toán',
    received: 'Nhận được',
    status: 'Trạng thái',
    discount: 'Giảm giá',
    estimatedAgx: 'Dự kiến AGX',

    tx: 'Giao dịch',
    title: 'Danh hiệu sáng lập',

    personalContribution: 'Đăng ký cá nhân',
    totalVolume: 'Tổng doanh số',
    bonusRate: 'Tỷ lệ phần thưởng',
    postLaunchRank: 'Sau ra mắt',
    amount: 'Số tiền',
    from: 'Địa chỉ nguồn',
    source: 'Nguồn',
    teamVolume: 'Doanh số nhóm',
    joined: 'Thời gian tham gia',
    address: 'Địa chỉ',
    direct: 'Trực tiếp',
    personal: 'Cá nhân',
    team: 'Nhóm',
    volume: 'Doanh số',

    contribution: 'Đăng ký',
    rate: 'Tỷ lệ',
  },
  status: {
    loading: 'Đang tải…',
    signInRequired: 'Đăng nhập bằng chữ ký để xem dữ liệu cá nhân',
  },
} satisfies AppMessagesBundle)

export default app
