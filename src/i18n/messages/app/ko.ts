    ...
    burn: {
      ...
      faq: {
        items: [
          {
            q: '기여 포인트는 어디에 쓰이나요?',
            a: '스테이킹·채권 등 수익을 수령할 때 기여 포인트를 {ratio}로 소모합니다. 포인트가 부족하면 수령할 수 없습니다.',
          },
          {
            q: '수익 수령에 기여 포인트가 필요한 이유는?',
            a: '수령을 프로토콜 디플레이션에 묶는 장치입니다. 수익을 수령할 때마다 기여 포인트를 {ratio}로 소모하며, 포인트는 AGX 소각으로만 얻을 수 있습니다. 따라서 수익을 인출할 때마다 AGX 소각이 대응되어, AGX 디플레이션을 지속적으로 뒷받침합니다.',
          },
          {
            q: '소각 비율은 얼마인가요?',
            a: '{burnRatio} 비율로 소각합니다. AGX 1개를 소각할 때마다 해당하는 기여 포인트를 얻습니다. 소각된 AGX는 온체인에서 블랙홀과 LP로 분할됩니다.',
          },
          {
            q: '소각된 AGX는 어디로 가나요?',
    ...
    turbine: {
      ...
      faq: {
        items: [
          ...
          {
            q: '잠금 해제와 출금의 차이는?',
            a: '잠금 해제는 현재 가격으로 USD1을 써서 동일 수량의 AGX를 매수하고, 잠긴 gAGX를 풀어 쿨다운을 시작합니다. 출금은 쿨다운({cooldownHours}시간)이 끝난 뒤, 잠금 해제된 gAGX를 지갑으로 옮깁니다. 두 단계는 터빈 기록에 「잠금 해제」와 「출금」으로 표시됩니다.',
          },
          {
            q: '쿨다운은 얼마나 걸리나요?',
            a: '잠금 해제할 때마다 쿨다운이 시작됩니다. 현재 주기는 {cooldownHours}시간이며, 시장 상태에 따라 자동 조정됩니다. 종료 후 해당 gAGX를 지갑으로 출금할 수 있습니다.',
          },
          {
  ...
  rewards: {
    ...
    referral: {
      ...
      faq: {
        ...
        items: [
          ...
          {
            q: '추천 리워드는 어떻게 수령하나요?',
            a: '왼쪽 수령 패널에서 수령과 재예치 배분을 설정하세요. 수령분은 릴리스 풀에 들어가 선택한 주기로 선형 릴리스되고, 재예치분은 단일 토큰 스테이킹으로 바로 들어가 복리됩니다. 수령과 재예치 모두 기여 포인트를 {ratio}로 소모합니다.',
          },
          {
            q: '직접 추천 주소 수란?',
      ...
      faq: {
        ...
        items: [
          ...
          },
          {
            ...
            a: '왼쪽 수령 패널 상단에서 공동 구축 / 평월상을 전환한 뒤, 수령과 재예치 배분을 설정하세요. 수령분은 릴리스 풀에 들어가 선택한 주기로 선형 릴리스되고, 재예치분은 단일 토큰 스테이킹으로 바로 들어가 복리됩니다. 둘 다 기여 포인트를 {ratio}로 소모합니다.',
          },
          {
    ...
    faq: {
      ...
      items: [
        ...
        {
          q: '리워드 수령에 필요한 조건은?',
          a: '수령 시 기여 포인트를 {ratio}로 소모합니다. 포인트는 AGX를 소각해 얻으며, 부족하면 먼저 소각 페이지에서 확보하세요.',
        },
        {
          q: '수령한 리워드는 언제 입금되나요?',
        ...
        {
          q: '리워드는 언제 정산되나요?',
          a: '럭키 추첨은 매일 00:00(UTC)에 정산됩니다. 그 외 리워드는 Rebase를 따르며, 약 {hours}시간마다 같은 주기로 정산됩니다. 다음 지급 시각은 각 리워드 상세의 데이터 패널에서 확인할 수 있습니다.',
        },
        {
    ...
    hub: {
      ...
      faq: {
        ...
        items: [
          ...
          {
            q: '수익을 수령할 수 없는 이유는?',
            a: '수익 수령에는 기여값을 소모합니다. 계정 기여값이 부족하면 수령할 수 없습니다. 먼저 AGX를 구매·소각해 기여값을 얻은 뒤 자산 페이지로 돌아오세요. 기여값 메커니즘은 수익을 인출할 때마다 프로토콜 디플레이션에도 기여하도록 합니다.',
          },
          {
            q: '기여값은 어떻게 얻나요?',
            a: 'AGX를 구매·소각하면 기여값을 얻습니다. 수익 수령 시 {ratio}로 소모되므로, 수령할 수익에 맞춰 미리 충분히 준비하세요.',
          },
          {
            q: '수익 수령 시 릴리스 주기를 고르는 이유는?',
            a: '수령한 수익은 즉시 입금되지 않고, 선택한 주기로 선형 릴리스됩니다. 주기가 길수록 세율이 낮습니다: {taxSchedule}.',
          },
          {
            q: '수익 수령 후 어디로 가나요?',
          ...
          {
            q: '수익 재예치와 수령의 차이는?',
            a: '재예치는 릴리스 주기를 건너뛰고 수익이 단일 토큰 스테이킹으로 바로 들어가 복리를 이어갑니다. 세율도 더 유리하며({restakeTax}), 장기 참여자에게 맞습니다. 수령은 릴리스 주기에 걸쳐 지갑으로 입금되어 더 유연합니다.',
          },
          {
            q: '버퍼 풀이란?',
            a: '원금을 언스테이크하면 버퍼 풀에 들어가 {days}일 2차 선형 릴리스로 단기 집중 유출을 줄입니다. 버퍼의 「릴리스됨」 분은 언제든 지갑으로 상환할 수 있습니다.',
          },
        ],
      },
    },
      ...
      stake: {
        ...
        faq: {
          ...
          items: [
            {
              q: '수령과 상환의 차이는?',
              a: '수령은 수익용: 누적된 gAGX를 선택한 릴리스 주기로 받거나 재예치합니다. 상환은 원금용: 릴리스된 AGX 원금을 꺼내 {days}일 버퍼에서 2차 선형 릴리스한 뒤 지갑으로 들어갑니다.',
            },
            {
              q: '왜 스테이킹마다 따로 표시되나요?',
    ...
    xmine: {
      ...
      faq: [
        ...
        },
        {
          ...
          a: '잠금 해제 후 gAGX는 {days}일 블록 선형 릴리스로, 언스테이크 후 집중 매도 압력을 줄이고 장기 가치 캡처를 강화합니다.',
        },
        {
  ...
  release: {
    ...
    faq: {
      ...
      hub: [
        ...
        {
          q: '세율은 언제 차감되나요?',
          a: '세율은 수익이 릴리스 풀에 들어갈 때, 선택한 주기 요율로 한 번만 차감됩니다({taxSchedule}). 풀에 표시되는 수량은 이미 세후이며, 릴리스와 이후 수령에 추가 수수료는 없습니다.',
        },
        {
      ...
      buffer: [
        {
          q: '버퍼 풀이란?',
          a: '원금을 언스테이크(상환)하면 버퍼 풀에 들어가 {days}일 2차 선형 릴리스가 진행됩니다. 단기 집중 유출을 줄여, 지속 릴리스와 시장 안정의 균형을 맞춥니다.',
        },
        {
          q: '버퍼 풀 자산에도 수익이 있나요?',
