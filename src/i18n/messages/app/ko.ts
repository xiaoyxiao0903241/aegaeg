...
const app = defineMessages({
  ...
  nav: {
    ...
    proposalTooltip: '프로토콜 제안에 투표하세요.',
  ...
  dapp: {
    connect: {
      ...
      recordsBodyProposal: '연결 후 제안 투표 기록이 여기에 표시됩니다.',
    ...
    burn: {
      ...
      faq: {
        items: [
          ...
          {
            q: '소각된 AGX는 어디로 가나요?',
            a: '소각된 AGX의 50%는 블랙홀 주소로 이전되어 영구 잠깁니다. 유통량을 직접 줄이고 디플레이션을 강화하며, 프로토콜 가치 환류 메커니즘의 일부입니다. 나머지 50%는 생태계 토큰 X 베이스 풀에 주입되어 X 유동성 깊이와 풀 안정성을 강화하고, 「수익 → 소각 → 유동성 강화」의 가치 선순환을 만듭니다.',
          },
          {
  ...
  rewards: {
    ...
    cards: {
      ...
      grant: {
        ...
        body: '생태계 발전 특별 수당',
        aside:
          '발전 수당은 AGX로 지급되며 승인 없이 MarketFund 서명으로 수령해 지갑으로 바로 입금됩니다.',
      },
      genesis: {
    ...
    grant: {
      claimIntoWallet: '지갑으로 바로 입금',
      claimHint:
        '발전 수당은 AGX로 지급되며 승인 없이 언제든 지갑으로 수령할 수 있습니다. 기여 포인트를 소모하지 않으며 릴리스 풀과 터빈도 거치지 않습니다.',
      ...
      faq: {
        ...
        items: [
          ...
          {
            q: '발전 수당은 어떤 형태로 지급되나요?',
            a: 'AGX로 지급됩니다. 누적되면 계정에 바로 들어가며 승인 없이 언제든 지갑으로 수령할 수 있습니다.',
          },
          {
            q: '수당 수령에 승인이 필요한가요?',
            a: '필요하지 않습니다. 지급되면 바로 수령 가능 상태이며, 수령 버튼을 누르면 AGX가 지갑으로 들어갑니다. 고객지원에 잠금 해제를 신청할 필요가 없습니다.',
          },
          {
            q: '수당 수령 시 기여 포인트를 소모하나요?',
            a: '필요하지 않습니다. 발전 수당은 다른 리워드와 달리 수령 시 기여 포인트를 소모하지 않고 릴리스 풀과 터빈도 거치지 않으며, AGX가 지갑으로 바로 입금됩니다.',
          },
          {
            q: '발전 수당은 어디에 쓸 수 있나요?',
            a: '시장 개발 전용입니다. 오프라인 살롱·로드쇼, 커뮤니티 운영·홍보 자료, 채널 확장 등 생태계 발전의 실제 필요에 맞게 사용하고, 이후 확인할 수 있도록 관련 기록을 남겨 두세요.',
          },
          {
            q: '수당 수량은 어떻게 계산되나요?',
            a: '팀 스테이킹 포지션 변동과 공동 구축 등급에 해당하는 수당 비율로 산정됩니다. 팀이 새로 스테이킹하면 비율만큼 누적되고, 팀이 상환하면 해당 비율이 누적액에서 차감됩니다. 상세는 수당 기록의 지급 탭에서 확인할 수 있습니다.',
          },
          {
            q: '수당 수량이 줄어든 이유는?',
            a: '팀 멤버가 포지션을 상환하면 해당 비율이 누적액에서 차감되며, 지급 기록에 「상환」 유형의 음수로 표시됩니다. 정상적인 정산 방식입니다.',
          },
        ],
      },
    },
  ...
  staking: {
    ...
    blocked: {
      ...
      insufficientQuotaWithAmount:
        '스테이킹 가능 한도를 초과했습니다: 현재 최대 {quota}까지 더 스테이킹할 수 있습니다. 수량을 줄인 뒤 다시 시도해 주세요.',
      insufficientQuotaPersonalWithAmount:
        '본인 스테이킹 가능 한도를 초과했습니다: 개인 누적 한도가 {quota} 남았습니다. 수량을 줄인 뒤 다시 시도해 주세요.',
      insufficientQuotaPersonalDailyWithAmount:
        '오늘 스테이킹 가능 한도를 초과했습니다: 오늘 개인 한도가 {quota} 남았습니다. 수량을 줄이거나 한도가 갱신될 때까지 기다려 주세요.',
      insufficientQuotaPoolWithAmount:
        '온체인 스테이킹 풀 한도가 부족합니다: 풀에 현재 {quota}가 남아 있습니다. 수량을 줄이거나 나중에 다시 시도해 주세요.',
      insufficientXmineQuotaWithAmount:
        '채굴 한도를 초과했습니다: 채굴 한도는 잠긴 원금으로 정해지며, 현재 최대 {quota}까지 더 스테이킹할 수 있습니다. 수량을 줄이거나 먼저 잠금 포지션을 늘린 뒤 다시 시도해 주세요.',
    ...
    stake: {
      ...
      amountAria: '스테이킹 수량',
      amountBalance: '수량(지갑 잔액 {balance})',
      quotaInline: '스테이킹 한도: {quota}',
      submit: '스테이킹',
      bindCta: '추천 바인딩으로 이동',
      success: '스테이킹 성공',
      periods: {
