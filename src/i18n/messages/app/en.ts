import { defineMessages } from '~/i18n/messages/define-messages'
import type { AppMessagesBundle } from './types'

const app = defineMessages({

  common: {
    brand: 'AEGIS X',
    bsc: 'BSC',
    connectWallet: 'Connect Wallet',
    language: 'Language',
    copy: 'Copy',
    success: 'Success',
    paid: 'Paid',
    claimable: 'Claimable',
    current: 'Current',
    max: 'Max',
    collapse: 'Collapse',
    expand: 'Expand',
    confirm: 'Confirm',
    close: 'Close',
    paginationTotal: '{total} total',
    paginationPerPage: '{size} per page',
    paginationPrev: 'Previous page',
    paginationNext: 'Next page',
  },
  nav: {
    swap: 'Swap',
    genesis: 'Co-build',
    rewards: 'Rewards',
    community: 'Community',
    genesisTooltip: 'Join Genesis subscription seasons and earn shareholder titles.',
    rewardsTooltip: 'View referral rewards and team rewards.',
    communityTooltip: 'Invite friends, bind a referrer, and grow your team.',
    bscTooltip:
      'BSC only · AEGIS X runs on BNB Smart Chain. Network switching is not supported yet.',
  },
  topbar: {
    currentNetwork: 'Current network',
    openMenu: 'Open navigation',
    closeMenu: 'Close navigation',
    hideDetails: 'Hide details panel',
    showDetails: 'Show details panel',
    toggleTooltip: 'Show or hide the details panel',
  },
  wallet: {
    connectTitle: 'Connect Wallet',
    connectIntroTitle: 'Connect your wallet to explore AEGIS X features',
    connectIntroLink: 'AEGIS X features ↗',
    installedWallets: 'Installed wallets',
    suggestedWallets: 'Suggested wallets',
    connectAction: 'Connect',
    connecting: 'Connecting…',
    termsPrefix: 'By connecting, you agree to our',
    termsLink: 'Terms of Service',
    viewAllWallets: 'View all wallets',
    allWalletsTitle: 'All wallets',
    scanQrInstruction: 'Scan this QR code with your wallet app to connect.',
    copyQrLink: 'Copy link',
    qrLinkCopied: 'Copied',
    connectingWallet: 'Waiting for wallet confirmation…',
    openWalletApp: 'Opening your wallet app…',
    connectionFailed: 'Connection failed',
    tryAgain: 'Try again',
    supportedNetwork: 'Supported network',
    copyAddress: 'Copy address',
    copied: 'Copied',
    disconnect: 'Disconnect',
    reconnectWallet: 'Reconnect wallet',
    reconnectHint: 'Wallet disconnected. Reconnect before performing on-chain actions.',
    signInRequired: 'Sign in',
  },
  swap: {
    title: 'Swap',
    intro: 'Real-time PancakeSwap market rates, settled on-chain in seconds.',
    disconnectedIntro: 'USDT ↔ USD1 · Fixed 1:1 · Zero slippage.',
    sell: 'Sell',
    buy: 'Buy',
    flip: 'Flip swap direction',
    balance: 'Balance',
    rate: 'Exchange rate',
    exchangePrice: 'Exchange price',
    ratePlaceholder: '1 : 1',
    slippage: 'Slippage tolerance',
    allowedSlippage: 'Allowed slippage',
    slippageSettings: 'Slippage tolerance settings',
    route: 'Swap route',
    provider: 'Provider',
    providerName: 'PancakeSwap',
    openPancakeSwap: 'Open on PancakeSwap',
    approve: 'Approve',
    action: 'Swap',
    approveSuccess: 'Approval successful',
    swapSuccess: 'Swap successful',
    connectToSwap: 'Connect wallet to swap',
    connectExploreTitle: 'Connect to explore AEGIS X features',
    connectExploreAction: 'Features available after connecting →',
    overview: 'Overview',
    exchangeRate: 'Exchange rate',
    fixedRate: '1 : 1 fixed',
    fixedRateConnected: '1 : 1.001',
    fixedRateHint: 'USDT ↔ USD1 · No protocol spread',
    settlement: 'Settlement',
    settlementValue: 'PancakeSwap · Instant settlement',
    settlementHint: 'BSC only · Gas paid by connected wallet',
    about: {
      title: 'About',
      body: 'USDT and USD1 swap instantly at a fixed 1:1 rate with zero slippage. USD1 is the core settlement asset of the AEGIS X protocol.',
    },
    tokenAbout: {
      title: 'About AEGIS X ecosystem tokens',
      items: [
        {
          key: 'usd1',
          title: 'USD1 · Core settlement asset',
          body: 'The core settlement asset of the AEGIS X ecosystem, connecting value circulation, liquidity networks, and payment scenarios.',
        },
        {
          key: 'agx',
          title: 'AGX · Core protocol asset',
          body: 'AGX is the core asset of the AEGIS X protocol, generated through a 150% over-collateralization mechanism, and plays a key role in value growth, yield distribution, and ecosystem development.',
          bodyDesktop:
            'The core governance and value token of the AEGIS X protocol, used for governance voting, staking, and ecosystem incentives, carrying long-term protocol growth value.',
        },
        {
          key: 'gagx',
          title: 'gAGX · Reward settlement voucher',
          body: 'A protocol reward settlement voucher redeemable for AGX and used in ecosystem mining and yield recycling.',
          bodyDesktop:
            'An interest-bearing voucher earned by staking AGX, with auto-compounding yields, unlocking governance weight and higher titles.',
        },
        {
          key: 'x',
          title: 'X · Ecosystem value token',
          body: 'The AEGIS X ecosystem value carrier with a fixed supply of 210 million, carrying ecosystem growth and value accumulation.',
          bodyDesktop:
            'An ecosystem participation and equity token that records on-chain contributions, redeemable for benefits, event access, and airdrop multipliers.',
        },
      ],
    },
    tokenContract: 'View contract',
    tokenPrevious: 'Previous token',
    tokenNext: 'Next token',
    swipeNext: 'Swipe up for next page',
    swipePrevious: 'Swipe down for previous page',
    faq: {
      title: 'FAQ',
      tabs: {
        usd1: {
          label: 'USD1',
          items: [
            {
              q: 'What is USD1?',
              a: 'USD1 is the core value settlement asset of the AEGIS X ecosystem. USD1 is backed 100% by reserve assets, including cash, short-term U.S. Treasuries, government money market funds, and more. Monthly distribution reports are available on the WLFI website.',
            },
            {
              q: 'What role does USD1 play in AEGIS X?',
              a: 'USD1 serves as the core settlement asset, connecting liquidity networks, payment scenarios, and ecosystem value flows.',
            },
            {
              q: 'How do I swap USD1?',
              a: 'Users can quickly swap USDT for USD1 through on-chain swap functionality to participate in the AEGIS X ecosystem.',
            },
          ],
        },
        agx: {
          label: 'AGX',
          items: [
            {
              q: 'What is AGX?',
              a: 'AGX is the governance and value token of AEGIS X, used for governance voting, staking, and ecosystem incentives.',
            },
            {
              q: 'What role does USD1 play in the protocol?',
              a: 'USD1 is the settlement rail for subscription, rewards, and swap flows, keeping value accounting simple and auditable.',
            },
            {
              q: 'How are USDT and USD1 swapped?',
              a: 'The app completes conversion through supported BSC swap routes, with the connected wallet confirming the final transaction.',
            },
          ],
        },
        gagx: {
          label: 'gAGX',
          items: [
            {
              q: 'What is gAGX?',
              a: 'gAGX is an interest-bearing voucher earned by staking AGX, unlocking governance weight and higher shareholder titles.',
            },
            {
              q: 'What role does USD1 play in the protocol?',
              a: 'USD1 is the settlement rail for subscription, rewards, and swap flows, keeping value accounting simple and auditable.',
            },
            {
              q: 'How are USDT and USD1 swapped?',
              a: 'The app completes conversion through supported BSC swap routes, with the connected wallet confirming the final transaction.',
            },
          ],
        },
        x: {
          label: 'X',
          items: [
            {
              q: 'What is X?',
              a: 'X is an ecosystem participation equity token that records on-chain contributions and qualifies for airdrops and event multipliers.',
            },
            {
              q: 'What role does USD1 play in the protocol?',
              a: 'USD1 is the settlement rail for subscription, rewards, and swap flows, keeping value accounting simple and auditable.',
            },
            {
              q: 'How are USDT and USD1 swapped?',
              a: 'The app completes conversion through supported BSC swap routes, with the connected wallet confirming the final transaction.',
            },
          ],
        },
      },
    },
    tokenContractTooltip: 'View token and contract details',
  },
  genesis: {
    title: 'Co-build Program',
    intro: 'Join the X DAO co-build program · Season {season} ({discount} off)',
    shares: 'Shares (1 share = 100 USD1 · max 100)',
    hint: 'Up to 32 shares left this season · 40 remaining network-wide',
    quota: 'Season co-build quota',
    pay: 'Pay',
    receive: 'You will receive AGX',
    value: 'Subscription value',
    xTokenAirdrop: 'Est. initial X airdrop value',
    xTokenAirdropHint:
      'Airdrop rewards require cumulative co-build participation ≥ 5,000 USD per account per season.',
    join: 'Join co-build',
    joinGenesis: 'Join Genesis',
    statsTitle: 'Season {season} co-build stats',
    startsIn: 'Starts in',
    endsIn: 'Time remaining this season',
    referencePrice: 'AGX launch reference price',
    contribution: 'Network subscription',
    discountLabel: 'Discount',
    discountRatio: 'Season discount rate',
    network: 'Network',
    xAirdropRatio: 'X airdrop ratio',
    airdropLabel: 'X airdrop ratio',
    addresses: 'Addresses',
    participants: 'Genesis participants',
    seasons: 'Subscription seasons',
    myContributions: 'My co-build records',
    connectBrandLine: 'Guard the future value network',
    contributionsConnectTitle: 'Connect your wallet to view your records',
    contributionsConnectBody: 'After connecting, your co-build history will appear here.',
    totalContributed: 'Total co-build',
    globalLabel: 'Global cumulative co-build',
    globalBody: 'Core co-builders worldwide are coming together to build the AEGIS X global ecosystem.',
    viewContract: 'View contract',
    faq: {
      title: 'FAQ',
      items: [
        {
          q: 'How do I join the co-build program?',
          a: 'Users participate in co-build with USD1 and receive AGX at the corresponding season discount. There are three seasons, each open for 20 days, with discounts of 30% / 25% / 20% respectively.',
        },
        {
          q: 'What are the quota and participation requirements?',
          a: 'Minimum participation is 100 USD1 in increments of 100 USD1. Season 1: $100–$10,000; Season 2: $100–$20,000; Season 3: $100–$30,000.',
        },
        {
          q: 'How long is the co-build vesting period?',
          a: 'AGX earned from co-build follows a 540-day release schedule.',
        },
        {
          q: 'How do I qualify for X airdrop rewards?',
          a: 'Accounts with cumulative co-build participation of 5,000 USD qualify for the corresponding season X airdrop. Airdrop ratios are 5% / 2% / 1% across the three seasons.',
        },
        {
          q: 'How are X airdrop rewards released?',
          a: 'X airdrop rewards vest linearly over 12 months, with approximately 8.33% released each month. The first release occurs 30 days after the X staking protocol goes live, executed automatically by smart contract.',
        },
      ],
    },
    promoTitle: 'Genesis Season 1 · 30% off',
    promoBody: 'Live now — limited quota, ends 07.10',
    promoTitleTemplate: 'Genesis Season {season} · {discount}',
    promoLive: 'Live now — limited quota, ends {endDate}',
    promoUpcoming: 'Coming soon — limited quota, starts {startDate}',
    promoEnded: '{status} · {date}',
    promoLoading: 'Loading season info…',
    joinSuccess: 'Subscription successful',
    approved: 'Approved',
    insufficientUsd1: 'Insufficient USD1 balance. Obtain enough USD1 before subscribing.',
    insufficientAllowance: 'Insufficient USD1 allowance. Approve first.',
    purchaseUnavailable: 'Subscription is currently unavailable. Check your shares or season status.',
    walletNotConnected: 'Wallet disconnected. Reconnect before signing transactions.',
    contributionsSyncPending: 'On-chain subscription confirmed. History is syncing — refresh shortly.',
    contributionsEmpty: {
      title: 'No co-build records yet',
    },
    usd1Balance: 'Co-build value',
    phase: 'Est. X airdrop',
    phaseSeason: 'Season {season} · {discount}',
    seasonLive: 'Live',
    seasonEnded: 'Ended',
    seasonUpcoming: 'Upcoming',
  },
  rewards: {
    title: 'Co-build Rewards',
    intro: 'Join co-build · Share growth value',
    currentTitle: 'Current tier',
    shareholder: 'S2 · Genesis Shareholder',
    shareholderHint: '2% team subscription reward · A3 title for 30 days after launch',
    shareholderHintForRank: '{bonus} team subscription reward · {postLaunch} title for 30 days after launch',
    shareholderHintNoRank: 'Genesis tier',
    shareholderNoRankTitle: 'Not yet a Genesis shareholder',
    shareholderNoRankBody:
      'Become a Genesis shareholder to earn 1%-10% of team co-build volume as rewards and upgrade one tier within 30 days after AEGIS X launch.',
    shareholderTitleForRank: '{rank} · Genesis Shareholder',
    heroKicker: 'Genesis tier',
    heroBody: 'Earn a {bonus} reward on team subscription volume and receive the {postLaunch} builder title for 30 days after launch.',
    heroBodyForRank:
      'Earn a {bonus} reward on team subscription volume and receive the {postLaunch} builder title for 30 days after launch.',
    currentTierSuffix: 'Current',
    progressPersonalTo: 'Progress to {rank} · Personal subscription',
    progressTeamTo: 'Progress to {rank} · Team subscription',
    progressMaxPersonal: 'Maximum personal tier reached',
    progressMaxTeam: 'Maximum team tier reached',
    teamLegRequirement: 'Two {rank} legs',
    teamVolume: 'Organization volume',
    referralRewards: 'Direct referral rewards',
    autoPaidLabel: 'Auto-paid',
    autoPaid: 'Rewards settle automatically to your wallet',
    teamRewards: 'Tier rewards',
    claimed: 'Claimed {amount}',
    claim: 'Claim to wallet',
    heroTitle: 'Current tier',
    allTiers: 'Genesis honor system',
    history: 'Reward history',
    historyConnectBody: 'After connecting, your reward history will appear here.',
    referralHistory: 'Referral rewards',
    teamHistory: 'Team rewards',
    referralHistoryEmpty: {
      title: 'No direct referral reward records yet',
      body: 'Direct referral rewards appear here after your referees complete subscriptions during Genesis.',
    },
    teamHistoryEmpty: {
      title: 'No tier reward records yet',
      body: 'Tier reward settlement and claim records appear here once rewards are generated.',
    },
    faq: {
      title: 'FAQ',
      items: [
        {
          q: 'How are referral rewards calculated?',
          a: 'Referral rewards are 3%, settled through compressed equal-amount matching — only the matched equal-amount portion counts, empty accounts do not count toward reward tiers, and rewards settle automatically.',
        },
        {
          q: 'How do Genesis tiers advance?',
          a: 'Genesis tiers range from S1 to S10, determined by personal co-build amount and total organization volume. Higher tiers require dual-leg advancement conditions.',
        },
        {
          q: 'What is the tier upgrade reward?',
          a: 'Genesis tiers achieved during co-build are automatically upgraded by one tier after protocol launch, valid for 30 days, then revert to the actual tier.',
        },
        {
          q: 'How are Genesis team rewards settled?',
          a: 'Genesis team rewards are calculated by tier rate. Smart contracts settle automatically daily at 00:00 UTC; users claim to wallet manually as needed. After co-build ends, this page closes — unclaimed rewards cannot be claimed and are routed to the smart market-making contract.',
        },
      ],
    },
    teamHistorySource: 'Period settlement',
    rewardType: {
      referralPaid: 'Referral reward',
      referralWithdrawn: 'Referral reward claim',
      marketTeam: 'Market-making team reward',
      presaleTeam: 'Presale team reward',
      unknown: '—',
    },
    logStatus: {
      pending: 'Pending',
      processing: 'Processing',
      paid: 'Paid',
      claimed: 'Claimed',
      failed: 'Failed',
      unknown: '—',
    },
  },
  community: {
    title: 'Community',
    intro: 'Invite partners to co-build and share ecosystem growth value and Genesis rewards.',
    disconnectedIntro: 'Connect your wallet to generate a referral link and bind an inviter.',
    referralLink: 'My referral link',
    shareReferral: 'Copy link',
    referrer: 'My inviter',
    bindReferrer: 'Bind',
    referrerPlaceholder: 'Enter referrer address (0x…)',
    referrerHint: 'Referral relationships are permanent once activated and cannot be changed.',
    docs: 'Resources',
    twitter: 'Twitter / X',
    telegram: 'Telegram',
    shareholder: 'Join co-build',
    myCommunity: 'My community',
    directReferrals: 'Direct referrals',
    myTeam: 'Community members',
    genesisTitle: 'Current',
    inviteTitle: 'Start inviting · Share ecosystem growth value',
    programs: {
      title: 'Ecosystem support programs',
      items: [
        {
          label: 'X DAO Co-build · Season 1',
          title: 'Global co-build program is live',
          body: 'Co-builders worldwide are coming together to build the ecosystem.',
          action: 'View program details →',
        },
        {
          label: 'X Academy',
          title: 'Ecosystem training program for co-builders',
          body: 'Helps co-builders gain a deeper understanding of ecosystem mechanics and development plans.',
          action: 'View program details →',
        },
      ],
    },
    myInvites: 'My community members ({count})',

    referralBondPermanent: 'Referral relationship active · binding is permanent.',
    referralBondActiveSince: 'Referral relationship active since {date} · binding is permanent.',
    volumePrefix: 'Volume',
    genesisShareholder: 'Genesis Shareholder',
    statDirectVolume: 'Volume $48,200',

    statTeamVolume: 'Volume $412,900',

    statGenesisVolume: 'Genesis Shareholder',
    statDirectToday: 'Today +2 · +$1,200',
    statTeamToday: 'Today +9 · +$8,640',
    statGenesisToday: 'Auto-upgrade +1 tier after launch',

    bindReferrerSuccess: 'Referrer bound successfully',
    inviteFlow: {
      items: [
        {
          title: 'Share your referral link',
          body: 'Connect your wallet and enter your inviter to generate your personal referral link.',
        },
        {
          title: 'Partners join co-build',
          body: 'After partners register through your referral link, they can participate in co-build.',
        },
        {
          title: 'Earn co-build rewards',
          body: 'After partners participate in co-build, rewards are settled automatically to your wallet address by smart contract.',
        },
      ],
    },
    boundTo: 'Bound to {address}',
    invitesEmpty: {
      title: 'No referral records yet',
      body: 'Share your referral link to invite friends to your community.',
    },
    faq: {
      title: 'FAQ',
      items: [
        { q: 'FAQ1', a: '---' },
        { q: 'FAQ2', a: '---' },
      ],
    },
  },
  tables: {
    time: 'Time',
    paid: 'Paid',
    received: 'Received',
    status: 'Status',
    discount: 'Discount',
    estimatedAgx: 'Est. AGX',

    tx: 'Transaction',
    title: 'Genesis title',

    personalContribution: 'Personal subscription',
    totalVolume: 'Total volume',
    bonusRate: 'Reward rate',
    postLaunchRank: 'After launch',
    amount: 'Amount',
    from: 'Source address',
    source: 'Source',
    teamVolume: 'Team volume',
    joined: 'Joined',
    address: 'Address',
    direct: 'Direct',
    personal: 'Personal',
    team: 'Team',
    volume: 'Volume',

    contribution: 'Subscription',
    rate: 'Rate',
  },
  status: {
    loading: 'Loading…',
    signInRequired: 'Sign in to view your personal data',
  },
} satisfies AppMessagesBundle)

export default app
