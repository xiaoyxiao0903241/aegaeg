    ...
    burn: {
      ...
      faq: {
        items: [
          {
            q: '贡献点数有什么用？',
            a: '领取质押、债券等各类收益时需按 {ratio} 消耗贡献点数。没有足够的贡献点数将无法领取收益。',
          },
          {
            q: '为什么领取收益需要消耗贡献点数？',
            a: '这是将收益领取与协议通缩绑定的机制：每领取收益需按 {ratio} 消耗贡献点数，而贡献点数只能通过销毁 AGX 获得。因此每一份收益的提取都对应 AGX 的销毁，持续为 AGX 提供通缩支撑。',
          },
          {
            q: '销毁比率是多少？',
            a: '按 {burnRatio} 比率销毁：每销毁 1 枚 AGX，获得对应贡献点数。销毁的 AGX 按链上拆分进入黑洞与 LP。',
          },
          {
            q: '销毁的 AGX 去了哪里？',
    ...
    turbine: {
      ...
      faq: {
        items: [
          ...
          {
            q: '解锁和提取有什么区别？',
            a: '解锁是以 USD1 按当前价格买入等量 AGX、使锁定的 gAGX 解除锁定并进入冷却期；提取是在冷却期（{cooldownHours} 小时）结束后，将已解锁的 gAGX 正式转入钱包。两步在涡轮记录中分别显示为「解锁」与「提取」。',
          },
          {
            q: '冷却时间是多久？',
            a: '每次解锁后进入冷却期，当前周期为 {cooldownHours} 小时，时长由系统根据市场状态自动调节；冷却结束后即可将该笔 gAGX 提取至钱包。',
          },
          {
            q: '解锁买入的 AGX 去了哪里？',
  ...
  rewards: {
    ...
    cobuild: {
      ...
      faq: {
        ...
        items: [
          ...
          {
            q: '共建奖和平越奖如何领取？',
            a: '在左侧领取面板顶部切换共建奖 / 平越奖，选择领取与复投的比例：领取部分进入释放池按所选周期线性释放，复投部分直接进入单币质押复利。领取和复投均按 {ratio} 消耗贡献点数。',
          },
          {
            q: '等级变化后奖金比例何时生效？',
    ...
    faq: {
      ...
      items: [
        ...
        {
          q: '领取奖励需要什么条件？',
          a: '领取奖励需按 {ratio} 消耗贡献点数。贡献点数通过销毁 AGX 获得，不足时请先前往销毁页面获取。',
        },
        {
          q: '领取的奖励什么时候到账？',
        ...
        {
          q: '奖励什么时候结算？',
          a: '幸运奖每日 00:00（UTC）结算；其余奖励跟随 Rebase 发放，Rebase 约每 {hours} 小时发放一次，因此奖励也随之每 {hours} 小时结算一次。下一次发放时间可在各奖项详情页的数据面板查看。',
        },
        {
          q: '为什么有的奖励卡片不显示？',
    ...
    hub: {
      ...
      faq: {
        ...
        items: [
          ...
          {
            q: '如何获得贡献值？',
            a: '购买 AGX 并进行销毁即可获得贡献值。领取收益时按 {ratio} 消耗贡献值，建议根据待领取的收益数量提前准备足够的贡献值。',
          },
          {
            q: '领取收益时为什么要选择释放周期？',
            a: '领取的收益不会立即到账，而是按所选周期线性释放，周期越长税率越低：{taxSchedule}。',
          },
          {
            q: '收益领取后去到了哪里？',
          ...
          {
            q: '收益复投和领取有什么区别？',
            a: '复投跳过释放周期，收益直接进入单币质押继续复利，税率更优（{restakeTax}），适合长期参与者；领取则按释放周期逐步到账钱包，灵活性更高。',
          },
          {
            q: '缓冲池是什么？',
            a: '本金解除质押后进入缓冲池，进行 {days} 天的二次线性释放，以降低短期集中流出对市场流动性的冲击。缓冲池中"已释放"的部分可随时赎回至钱包。',
          },
        ],
      },
    },
      ...
      stake: {
        ...
        faq: {
          ...
          items: [
            {
              q: '领取和赎回有什么区别？',
              a: '领取针对收益：将累积的 gAGX 收益按所选释放周期领出，或直接复投；赎回针对本金：将已释放的 AGX 本金取出，进入 {days} 天缓冲区二次线性释放后到账钱包。',
            },
            {
              q: '为什么每笔质押单独显示？',
      ...
      lpbond: {
        ...
        faq: {
          ...
          items: [
            {
              q: '领取和赎回有什么区别？',
              a: '领取针对收益：将债券产生的 gAGX 收益按所选释放周期领出，或直接复投；赎回针对本金：将已释放的 AGX 本金取出，进入 {days} 天缓冲区二次线性释放后到账钱包。',
            },
            {
              q: '「债券本金」是怎么来的？',
            ...
            {
              q: '债券收益可以复投吗？',
              a: '可以。领取时通过滑条自由分配复投与领取的比例：复投部分进入所选周期（{restakeDays} 天）的单币质押继续复利，税率优于按周期领取。',
            },
            {
              q: '倒计时结束后会怎样？',
      ...
      xmine: {
        ...
        faq: {
          ...
          items: [
            {
              q: '领取产出和赎回质押有什么区别？',
              a: '领取针对挖矿产出：X 奖励可随时领取、无释放周期，直接进入您的钱包；赎回针对质押本金：gAGX 赎回后进入缓冲区，进行 {days} 天二次线性释放，缓冲区资产不再产生任何收益。',
            },
            {
              q: '为什么有的仓位显示「锁定」？',
  ...
  staking: {
    ...
    hub: {
      ...
      faq: {
        ...
        items: [
          {
            q: 'Rebase 如何结算？',
            a: '协议以区块为基础运行：约 {blocks} 个区块 = 1 个 Epoch（约 {hours} 小时）。每个 Epoch 结束时执行一次 Rebase 结算，系统每日进行 {timesPerDay} 次收益分配。',
          },
          {
            q: '本金如何释放？',
            a: '质押与债券本金采用区块级线性释放模型（约 3 秒一个区块连续释放结算）。已释放本金在提取后，将进入 {days} 天缓冲释放周期，通过双层线性释放平衡资金释放连续性与市场稳定性。',
          },
          {
            q: '质押、LP债券和销毁债券有什么区别？',
    ...
    stake: {
      ...
      faq: [
        {
          q: '质押收益如何计算？',
          a: '每日 {timesPerDay} 次 Rebase，日收益 0.5% – 1%。质押周期越长可获得更高加成：180 天 ≥10%、360 天 ≥15%、540 天 ≥20%，与 Rebase 系数联动调节。',
        },
        {
          q: '质押本金何时可取？',
          a: '本金按区块线性释放（约 3 秒一个区块），释放完成部分可随时提取；提取后进入 {days} 天缓冲释放周期。',
        },
        {
          q: '参考 APY 是固定的吗？',
    ...
    xmine: {
      ...
      faq: [
        ...
        {
          q: '解押后资产如何释放？',
          a: '解锁后的 gAGX 资产采用 {days} 日区块线性释放机制，降低解押后集中卖压风险，增强长期价值捕获能力。',
        },
        {
          q: 'X 的总量是多少？会增发吗？',
  ...
  release: {
    ...
    faq: {
      ...
      hub: [
        ...
        {
          q: '税率是什么时候扣除的？',
          a: '在收益进入释放池时按所选周期的税率一次性扣除（{taxSchedule}），池中显示的已是税后数量，释放与领取过程中不再产生额外费用。',
        },
        {
          q: '从释放池领取的 gAGX 去向哪里？',
      ...
      buffer: [
        {
          q: '缓冲池是什么？',
          a: '本金解除质押（赎回）后进入缓冲池，进行 {days} 天的二次线性释放，降低短期集中流出对市场流动性的冲击，实现资金释放的连续性与市场稳定性的平衡。',
        },
        {
          q: '缓冲池里的资产还有收益吗？',
