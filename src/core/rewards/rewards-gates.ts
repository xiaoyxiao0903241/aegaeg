/**
 * Pure live-gate helpers for rewards simple sign + Mixed (Lucky / DaoPool).
 * sessionReady ≠ empty/zero amount — separate reasons for honest UI copy.
 */

export type RewardsSimpleClaimGateReason = 'notSessionReady' | 'zeroAmount' | 'lockedUnknown'

export type RewardsMixedGateReason =
  | 'zeroAmount'
  | 'insufficientReward'
...
export function evaluateRewardsSimpleClaimGate(args: {
  sessionReady: boolean
  amount: bigint
  unknownLocked: boolean
}): RewardsSimpleClaimGateReason | null {
...
export function evaluateRewardsMixedClaimGate(args: {
  amount: bigint
