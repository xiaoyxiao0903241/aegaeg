...
export function resolveExchangeUserFacingMessage(
  error: unknown,
  messages: {
  ...
  if (
    (raw === EXCHANGE_QUOTE_FAILED || raw === EXCHANGE_SUBMIT_GATE_FAILED) &&
    messages.quoteFailed
  ) {
    return messages.quoteFailed
  }
  if (raw === WALLET_GATE_ERROR.NOT_CONNECTED || /wallet not connected/i.test(raw)) {
    return messages.walletNotConnected
  }

  const fallback =
    chainFallback ?? walletTransactionErrors?.transactionFailed ?? messages.purchaseUnavailable
